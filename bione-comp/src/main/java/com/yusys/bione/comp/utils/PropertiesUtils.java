...
public class PropertiesUtils {
	...
	private static Map<String, PropertiesUtils> propertiesMap = new ConcurrentHashMap<String, PropertiesUtils>();
	...
	public static PropertiesUtils get(String location) {
		PropertiesUtils utils = propertiesMap.get(location);
		InputStream is = null;
		try {
			Resource resource = resourceLoader.getResource(location);
			if (utils == null || utils.lastModified < resource.lastModified()) {
				// 载入文件, 文件路径使用Spring Resource格式.
				logger.debug("Loading properties file from:" + location);
				...
				if (utils == null) {
					...
					propertiesMap.put(location, utils);
				}
				utils.lastModified = resource.lastModified();
