...
#***************************************************
#
#脚本名称 : swap_next_zl.sh
#脚本功能 : 修改一体机上文件名由tmp为txt,将本次上传的
#           文件名信息存入ytj_next_bigfile_detail中
#创建日期 : 20190305
#作    者 : bwn
#版    本 : v_1.0
#
...
sh ${shPath}/insert_ytj_log_zl.sh "swap_next_zl.sh" ${oDataDate} 5
    ...
    if [ "${g_filenameTemp##*.}" != "txt" ];then
            ...
            ##获取数据文件对应数据日期
                ...
                upTaskTab=`sqlplus -S "${CONNSTR}"<<EOF
                           ...
                           set linesize 1000
                           var ret number;
                           insert into YTJ_NEXT_BIGFILE_DETAIL(data_date, up_file_name, change_file_name, status, create_time) 
