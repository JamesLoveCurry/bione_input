(function($) {
	...
	// 零值校验校验状态渲染  add by wusb at 20161207
	...
	GridObject = {
			...
			"applySts" : {display : '申请状态', name : 'applySts', minWidth : 100, width : 100}
	...
	var SearchFormObject = {
			...
			"taskNmMulti" : {display : "任务名称", name : "taskNmMulti", newline : false, type : "select",width : '140', cssClass : "field", labelWidth : '90',
				comboboxName : "taskNm_sel", attr : { op : "in", field : "i.task_id"}
			}
	};
	//生成子对象
	...
	function setTree(searchFlag){
		if(searchFlag){
			...
			loadTree(urlSync, taskOrgTree, {orgNm : $("#treeSearchInput").val()});
		}else{
	...
	TaskFillGrid = function(gridUrl, columnSource, isOnDbClick, callBack, checkbox , delayLoad ,sortName,operType) {
		...
		var eles = {height : '99%', width : '100%',
					...
					method : 'get', url : gridUrl, sortName : sortName?sortName:'i.data_date desc, i.exe_obj_id, i.task_obj_id, i.task_id', 
	...
	CommonSerchForm = function(taskComBoBoxUrl, rptComBoBoxUrl, orgTreeSkipUrl, lineComBoBoxUrl, fieldSource, flag, newLineNum,operType) {
		...
		if(taskComBoBoxUrl){
			if(SearchFormObject["taskNmMulti"]){
				SearchFormObject["taskNmMulti"].options = {
						isMultiSelect : true,
						isShowCheckBox : true,
						split : ",",
						valueFieldID : "taskId", 
