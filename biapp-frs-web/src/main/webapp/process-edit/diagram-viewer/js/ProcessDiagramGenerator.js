...
var ProcessDiagramGenerator = {	
	...
	showFlowInfo: function(flow){
		...
		var TPL_FLOW_INFO = "";/*'<div>{flow}</div>' +
				 '<div><b>起始位置</b>: {sourceActivityId}</div>'
				+ '<div><b>目的地位置</b>: {destinationActivityId}</div>'
	...
	showActivityInfo: function(activity){
		...
		var values = {
			...
			group : activity.getProperty("group"),
			user : activity.getProperty("user"),
			assignee : activity.getProperty("assignee")
		};
		var TPL_ACTIVITY_INFO = '';
		if (values.group) {
			TPL_ACTIVITY_INFO += '<div><b>节点执行岗</b>: ' + values.group + '</div>';
		}
		if (values.user) {
			TPL_ACTIVITY_INFO += '<div><b>节点执行候选人</b>: ' + values.user + '</div>';
		}
		if (values.assignee) {
			TPL_ACTIVITY_INFO += '<div><b>节点执行人</b>: ' + values.assignee + '</div>';
		}
