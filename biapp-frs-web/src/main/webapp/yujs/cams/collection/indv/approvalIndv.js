...
function onSaveSubmit() {
	...
	JSPFree.confirm('提示', '你确定数据审批通过吗?', function(_isOK){
		if (_isOK){
			var jsn_result = JSPFree.doClassMethodCall("com.yusys.cams.indv.service.CamsIndvBSDMO", "submitDataApproval", def);
...
function doUpdateBackStatus(_reason){
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.cams.indv.service.CamsIndvBSDMO","backDataApproval",jso_par);
