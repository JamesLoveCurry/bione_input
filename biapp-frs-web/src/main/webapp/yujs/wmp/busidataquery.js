//业务数据查询【/frs/yufreejs?js=/yujs/wmp/busidataquery.js】
function AfterInit(){
	JSPFree.createSplit("d1","左右",235);
	JSPFree.createBillTree("d1_A","/biapp-wmp/freexml/wmp/wmp_cr_tab_ref.xml"); //
	...
	JSPFree.bindBillTreeOnSelect(d1_A_BillTree,function(_node){
	  var isLeaf_1 = JSPFree.isBillTreeSelectNodeLeaf(d1_A_BillTree);  //13张表是否选中的是叶子结点
	  if(isLeaf_1){  //如果选中了叶子结点
	  		...
	  		var jso_data = JSPFree.doClassMethodCall("com.yusys.wmp.report.service.FrsWmpCrReportBS","checkTabCols",jso_par); 
	  		...
	  		JSPFree.createOrReplaceIFrame("d1_B","/yujs/wmp/busi13.js",v_par);
	  	}
	});
}
