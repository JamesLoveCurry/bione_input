//初始化界面,菜单配置路径是【/frs/yufreejs?js=/yujs/wmp/13table.js】
function AfterInit(){
	JSPFree.createBillList("d1","/biapp-wmp/freexml/wmp/wmp_cr_tab_CODE1.xml",null,{isSwitchQuery:"N"});
}
...
function maintainCols() {
	...
	//弹出窗口,传入参数,然后接收返回值!
	JSPFree.openDialog("表字段","/yujs/wmp/maintaincols.js",900,485,jso_OpenPars,function(_rtdata){
...
function checkCols() {
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.wmp.business.service.FrsWmpCrTabBS", "verificationData", jso_Pars);
...
function deleteTab(){
	...
	JSPFree.confirm('提示', '你真的要删除选中的记录吗?', function(_isOK){
		if (_isOK){
			var jso_Pars = {tabNameEn:json_data.tab_name_en,tabName:json_data.tab_name};
			var jsn_result = JSPFree.doClassMethodCall("com.yusys.wmp.business.service.FrsWmpCrTabBS", "deleteColsByTabName", jso_Pars);
