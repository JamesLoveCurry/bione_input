...
function AfterInit() {
	...
	var jso_CardConfig = {
			"afterloadclass": "com.yusys.safe.receiptRecord.service.AfterSafeReceiptDataLoadBS",
...
function onSave() {
	...
	if ("Y" == ischeck) {
		...
		if (backValue == "" || "undifind" == backValue) {
			...
			if (saveFlag) {
				// 处理校验结果表的数据状态
				var tab_name = SafeFreeUtil.getTableNames().SAFE_RECEIPT_DATA;
				// 获取英文表名
				...
				var jso_rt = JSPFree.doClassMethodCall("com.yusys.safe.receiptRecord.service.SafeReceiptRecordBS",
