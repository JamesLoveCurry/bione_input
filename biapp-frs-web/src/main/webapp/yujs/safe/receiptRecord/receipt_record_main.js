...
function AfterInit() {
	...
	// 获取【错误补录表】常量类
	tab_name = SafeFreeUtil.getTableNames().SAFE_RECEIPT_DATA;
	// 获取英文表名
	...
	JSPFree.createBillList("d1", str_className, null, {list_btns:"$VIEW;[icon-p41]单一处理/singleHandler(this)", isSwitchQuery:"N", orderbys:"result_status"});
}
...
function singleHandler() {
	...
	var def = {
		tablename_en: selectData.tablename_en,
		tablename: selectData.tablename, pkcolname: selectData.pkcolname,
		pkvalue: selectData.pkvalue, report_type: str_subfix
	};
	if (SafeFreeUtil.getSafeReceiptProcessingStatus().status_1 == selectData.result_status) {
		JSPFree.openDialog(selectData.tablename,"/yujs/safe/receiptRecord/receipt_record_handle.js", 900, 560, def,function(_rtdata){
