...
function AfterInit(){
  JSPFree.createSplitByBtn("d1","左右",620,["确定/onConfirm","取消/onCancel"]);
	var whereSql = '';
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.cr.business.service.CrGetReportBS", "getReportCNList", null);
	if (jsn_result.code == 'success') {
		whereSql = jsn_result.data;
	}
  //创建左边的表
  JSPFree.createBillList("d1_A","/biapp-cr/freexml/engineTask/cr_engine_rule_task_ref.xml",null,{isSwitchQuery:"N", refWhereSQL: whereSql});
  ...
  //创建右边的购物车表
  JSPFree.createBillListByItems("d1_B","/biapp-cr/freexml/engineTask/cr_engine_rule_task_ref.xml",ary_cartkeys,ary_cartnames,ary_cartwidths);
