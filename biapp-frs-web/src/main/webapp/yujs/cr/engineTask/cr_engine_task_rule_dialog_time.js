...
function AfterInit() {
	...
	JSPFree.createBillCard("d1", "/biapp-cr/freexml/engineTask/cr_engine_task_data_ref.xml", ["上一步/onBefore/icon-p71","确认/onNext/icon-p01","取消/onCancel/icon-no"]);
}
...
function onNext() {
	...
	JSPFree.confirm("提示","复制【" + str_fromDate + "】的任务到【" + str_toDate + "】，请确认!",function(_isOK){
		if (_isOK) {
			...
			var jso_data = JSPFree.doClassMethodCall("com.yusys.cr.engineTask.service.CrEngieTaskBS", "batchRuleCreate", jso_par);
			if (jso_data.code == "OK") {
