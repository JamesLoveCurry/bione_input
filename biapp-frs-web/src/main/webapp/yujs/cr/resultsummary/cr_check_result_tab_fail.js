...
function AfterInit() {
	...
	var jsoResult = JSPFree.doClassMethodCall("com.yusys.cr.result.service.CrResultSummaryBS","getErrorCountByTx", parJson);
	if (jsoResult.notSuperUser == 'Y') {
		...
		var str_className = "Class:com.yusys.cr.result.template.CrTabFailDetailTemplet.getTemplet('"+tabName+"','"+tabNameEn+"','"+dataDt+"','"+orgNo +"','"+orgType +"')";
...
function getTabTemplateSql(str_data_dt) {
	...
	var jso_sql = JSPFree.doClassMethodCall("com.yusys.cr.result.template.CrTabFailDetailTemplet","getTabTemplateSql", defvalue);
