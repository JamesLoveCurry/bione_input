 ...
 * <pre>
 * Title: 【引擎管理】
 * Description:【检核任务】--规则任务--复制任务--下一步
 * </pre>
 * @author wangxy31 
 * @version 1.00.00
   @date 2020年8月17日
...
function AfterInit() {
	...
	JSPFree.createBillCard("d1", "/biapp-imas/freexml/engineTask/imas_engine_task_data_ref.xml", ["上一步/onBefore/icon-p71","确认/onNext/icon-p01","取消/onCancel/icon-no"]);
}
...
function onNext() {
	...
	JSPFree.confirm("提示","复制【" + str_fromDate + "】的任务到【" + str_toDate + "】，请确认!",function(_isOK){
		if (_isOK) {
			...
			var jso_data = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "batchRuleCreate", jso_par);
			if (jso_data.code == "OK") {
