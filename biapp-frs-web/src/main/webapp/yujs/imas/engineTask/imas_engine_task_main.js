...
function AfterInit() {
	...
	JSPFree.createBillList("d1_1", "/biapp-imas/freexml/engineTask/imas_engine_tab_task_CODE1.xml", null, {isSwitchQuery:"N"}); // 第1个页签
	JSPFree.createBillList("d1_2", "/biapp-imas/freexml/engineTask/imas_engine_rule_task_CODE1.xml", null, {isSwitchQuery:"N"}); // 第2个页签
...
function extracted(selectData) {
	...
	var jso_status = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getBreakTaskStatus", jso_par);
	...
	JSPFree.confirm('提示', '是否中断任务?', function (_isOK) {
		if (_isOK) {
			var _rt = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "forceCloseEngineTask", {task_id: selectData.task_id});
			if (_rt.status == "OK") {
...
function getCondition() {
	...
	var whereSql = '';
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getListByCreateBy", {field: 'busi_org_no'});
 ...
 */
function tabCreate() {
	JSPFree.openDialog("引擎任务","/yujs/imas/engineTask/imas_engine_task_tab_add.js", 800, 540, {org_no: org_no, org_class: org_class}, function(_rtdata){
		if (_rtdata == true) {
 ...
 */
function ruleCreate() {
	JSPFree.openDialog("引擎任务","/yujs/imas/engineTask/imas_engine_task_rule_add.js", 800, 540, {org_no: org_no, org_class: org_class}, function(_rtdata){
		if (_rtdata == true) {
...
function tabUpdate() {
	...
	var jso_status = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getTaskStatus", jso_par);
	if (jso_status.code == -999) {
	...
	JSPFree.openDialog("引擎任务", "/yujs/imas/engineTask/imas_engine_task_tab_edit.js", 700, 520, {task_id:selectDatas[0].task_id,org_no: org_no, org_class: org_class}, function(_rtdata){
		if (_rtdata == true) {
			JSPFree.queryDataByConditon(d1_1_BillList, null);  // 立即查询刷新数据
		}
	});
...
function ruleUpdate() {
	...
	var jso_status = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getTaskStatus", jso_par);
	if (jso_status.code == -999) {
		$.messager.alert('提示', '当前规则任务正在进行或排队中，请刷新状态或不能编辑！', 'warning');
		
		return;
	}
	
	JSPFree.openDialog("引擎任务", "/yujs/imas/engineTask/imas_engine_task_rule_edit.js", 700, 520, {task_id:selectDatas[0].task_id,org_no: org_no, org_class: org_class}, function(_rtdata){
		if (_rtdata == true) {
			JSPFree.queryDataByConditon(d1_2_BillList, null);  // 立即查询刷新数据
		}
	});
...
function tabDelete() {
	...
	var jso_status = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getTaskStatusIsInConductByTaskIds", {allTaskIds: jso_rids});
...
function ruleDelete() {
	...
	var jso_status = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "getTaskStatusIsInConductByTaskIds", {allTaskIds: jso_rids});
	if (jso_status.msg == 'OK') {
		$.messager.alert('提示', '所选规则任务正在进行或排队中，请刷新状态或不能删除！', 'warning');
...
function tabStart () {
	...
	var jsy_data = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS","getTaskStatusIsInConductByTaskIds", {allTaskIds: jso_rids});
	if (jsy_data.msg == "OK") {
		$.messager.alert('提示', '所选任务中存在进行或排队中的任务!', 'warning');
		return;
	}
	var jso_data = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "startAllTask", {allTaskIds: jso_rids});
	if (jso_data.errorCode == '0000') {
		tabRefresh();
		JSPFree.openDialog2("任务监控","/yujs/imas/engineTask/imas_engine_task_start.js",1000, 600,null);
	} else {
...
function tabLog() {
	...
	JSPFree.openDialog("日志", "/yujs/imas/engineTask/imas_engine_task_log.js", 900, 600, jso_OpenPars, function(_rtdata) {});
}
...
function dequeue(selectDatas) {
	...
	var jsy_data = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS","getQueueTaskStatus", {allTaskIds: jso_rids});
	if (jsy_data.code == "success") {
		$.messager.alert('提示', '只能选择排队中的任务,或任务已经启动引擎还未返回状态,请稍后!', 'warning');
		return;
	}

	var jso_data = JSPFree.doClassMethodCall("com.yusys.imas.engineTask.service.ImasEngieTaskBS", "removeQueue", {allTaskIds: jso_rids});
