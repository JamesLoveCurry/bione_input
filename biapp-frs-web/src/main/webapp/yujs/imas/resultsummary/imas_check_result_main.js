...
function AfterInit() {
	...
	JSPFree.createBillList("d1_1", "/biapp-imas/freexml/resultsummary/imas_result_tab_summary_v.xml",null,{isSwitchQuery:"N",autoquery:"N",list_ispagebar:"Y"}); // 按表查询
	d1_1_BillList.pagerType="2"; //第二种分页类型
	JSPFree.billListBindCustQueryEvent(d1_1_BillList, onTabErrorSummary);
	
	JSPFree.createBillList("d1_2", "/biapp-imas/freexml/resultsummary/imas_result_rule_summary_v.xml",null,{isSwitchQuery:"N",autoquery:"N",list_ispagebar:"Y"}); // 按规则查询
...
function onRuleErrorSummary(_condition) {
	var rule_sql = "";
	var defvalue = {"_condition":_condition, "_loginUserOrgNo" : str_LoginUserOrgNo};
	var jso_sql = JSPFree.doClassMethodCall("com.yusys.imas.result.service.ImasResultSummaryBS","getRuleSummaryQuerySql", defvalue);
	if (jso_sql.result == "OK") {
...
function viewFailDetailByTab(_btn) {
	...
	// 查看详情前，先确认表是否存在
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.result.service.ImasResultSummaryBS", "checkFailTab", jso_OpenPars);
	
	if (jso_rt.msg == "OK") {
		JSPFree.openDialog("错误明细","/yujs/imas/resultsummary/imas_check_result_tab_fail.js", 950, 500, jso_OpenPars, function(_rtdata){});
	} else {
...
function viewFailDetailByRule(_btn) {
	...
	if (jso_rt.msg == "OK") {
		JSPFree.openDialog("错误明细","/yujs/imas/resultsummary/imas_check_result_rule_fail.js", 950, 600, jso_OpenPars, function(_rtdata){});
	} else {
		JSPFree.alert("当前明细表不存在，无法查看明细数据!");
	}
}
...
function historyByRule(_btn) {
	...
	JSPFree.openDialog("历史数据","/yujs/imas/resultsummary/imas_check_result_rule_history.js", 700, 450, jso_OpenPars,function(_rtdata){});
}
...
function exportFialDetailByTab(_btn){
	...
	if (_org_ot != null && _org_ot != "") {
		// 根据中文表名，找对应的英文code
		var jso_org = JSPFree.getHashVOs("SELECT org_no FROM rpt_org_info where org_type='IMAS' and is_org_report='Y' and org_no = '"+_org_ot+"'");
	...
	// 先确认表是否存在
	var jso_OpenPars = {tabName: row.tab_name,tabNameEn:row.tab_name_en,dataDt:row.data_dt, orgNo: org_no1, orgType: org_type, failCount: row.fail_count};
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.result.service.ImasResultSummaryBS", "checkFailTab", jso_OpenPars);
	...
	var whereSql = " data_dt='" + row.data_dt + "' and ";
