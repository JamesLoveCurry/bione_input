...
function AfterInit() {
	...
	JSPFree.createBillCard("d1_A", "/biapp-imas/freexml/report/imas_report_choose_date.xml");
	
	JSPFree.createBillList("d1_B", "/biapp-imas/freexml/common/imas_cr_tab_ref.xml", null, 
...
function onConfirm() {
	...
	//远程调用确认指标有数据
	var jso_check = JSPFree.doClassMethodCall("com.yusys.imas.report.service.ImasCrReportBS", "checkIdxStatus", jso_par);
	if (jso_check.code == "-999") {
	       JSPFree.alert(jso_check.msg);
	       return;
	}
	
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.report.service.ImasCrReportBS", "getIdx", jso_par);
	if(jso_rt.code == "-999"){
