...
function AfterInit() {
    ...
    JSPFree.createBillList("d1","/biapp-imas/freexml/fillingProcess/imas_lock_data_code.xml",null,{list_btns:"[icon-p38]新增/createLock;[icon-p18]锁定/onLock_1;[icon-p20]解锁/onUnLock_1;[icon-remove]删除/del",isSwitchQuery:"N"});
...
//创建任务
function createLock(){
    JSPFree.openDialog("新增","/yujs/imas/fillingProcess/lock_data_insert.js",500,500,{org_no:org_no,org_class:org_class,org_nm:org_nm,currSQL:d1_BillList.CurrSQL},function(_rtdata){
    	if (_rtdata != null) {
...
function onLock_1(){
	...
	JSPFree.openDialog("加锁方式","/yujs/imas/fillingProcess/lock_data_choseLock.js",400,200,{rid:rid,type:'1'},function(_rtdata){
        if (_rtdata != null) {
            if (_rtdata.status == "OK") {
            	JSPFree.refreshBillListCurrRows(d1_BillList);  //刷新当前行
...
function del(){
    ...
    $.messager.confirm('提示', '你真的要删除选中的记录吗?', function(_isConfirm) {
		...
		var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasLockData","del",{str_id : idStr});
	    if(jso_rt.msg=='ok'){
...
function onUnLock_1	(){
	...
	JSPFree.openDialog("解锁","/yujs/imas/fillingProcess/lock_data_choseLock.js",400,200,{rid:rid,type:'2'},function(_rtdata){
        if (_rtdata != null) {
            if (_rtdata.status == "OK") {
            	JSPFree.refreshBillListCurrRows(d1_BillList);  //刷新当前行
...
function doCommitUpdateDB(tab_name, data_dt, status){
    jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasLockData", "startLockData", {tab_name:tab_name,data_dt:data_dt,status:status});
