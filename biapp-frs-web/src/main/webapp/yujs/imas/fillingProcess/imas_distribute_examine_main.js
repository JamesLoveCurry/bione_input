...
function AfterInit(){
	...
	templeteCode = "/biapp-imas/freexml/fillingProcess/imas_filling_process_child_CODE3.xml";
	JSPFree.createBillList("d1",templeteCode,null,
			{isSwitchQuery:"N",querycontion:"status='1'",autocondition:"status='1'",autoquery:"N",list_ischeckstyle: "Y",
...
function getQueryCondition(){
	var condition = "";
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasCrProcessBSDMO","getQueryConditionByUser",{"_loginUserOrgNo" : str_LoginUserOrgNo});
...
function examine(){
	...
	//判断当前数据是否被锁定
	var lock_data = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasLockData", "getStatus", {tab_name:json_data[0].tab_name,data_dt:json_data[0].data_dt,org_no:json_data[0].rpt_org_no,type:'3'});
	...
	JSPFree.openDialog("审核","/yujs/imas/fillingProcess/imas_distribute_examine_edit.js",1150,750,jso_Pars,function(_rtData){
		if (_rtData == "通过") {
			// 从界面上删除行
...
function batchExamine(){
    ...
    $.messager.confirm('提示', '是否批量审核通过所选数据？', function(r) {
        if (r) {
            var jso_allrids=[];
            for (var i=0;i<json_data.length;i++) {
            ...
            var jsn_result = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasCrProcessBSDMO", "updateDataByTaskByRids3", jso_Pars);
