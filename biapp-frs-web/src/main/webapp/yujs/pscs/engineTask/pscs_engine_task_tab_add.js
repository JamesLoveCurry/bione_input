...
function AfterBodyLoad() {
	...
	var taskCmd = JSPFree.doClassMethodCall("com.yusys.pscs.engineTask.service.PscsEngieTaskBS", "getTaskCmd", {taskType: '表任务'});
	...
	if ($("#busi_org_no") && $("#busi_org_no").length > 0) {
		$("#busi_org_no").textbox({
			onChange: function (newValue, oldValue) {
				var org_class = PscsFreeUtil.getOrgClass(newValue)
				if (org_class == PscsFreeUtil.getPscsOrgClass().zh) {
					// 如果是总行，则进行赋值
					JSPFree.setBillCardItemValue(d1_BillCard, "busi_type", "1");
				} else if (org_class == PscsFreeUtil.getPscsOrgClass().fh) {
