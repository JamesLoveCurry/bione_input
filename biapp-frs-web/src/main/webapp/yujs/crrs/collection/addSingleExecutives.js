...
function AfterInit(){
	...
	JSPFree.createBillCard("d1_A","/biapp-crrs/freexml/crrs/customer/crrs_single_executives_CODE1.xml",["保存/onSave/icon-ok"],null);
	//赋值
	...
	JSPFree.createBillList("d1_B_A","/biapp-crrs/freexml/crrs/group/crrs_passport_CODE1.xml",null,{autoquery:"N",ishavebillquery:"N",list_ispagebar:"N"}); // autocondition:"executives_id='"+executiveId+"'",querycontion:"executives_id='"+executiveId+"'"
	JSPFree.createBillList("d1_B_B","/biapp-crrs/freexml/crrs/group/crrs_paper_CODE1.xml",null,{autoquery:"N",ishavebillquery:"N",list_ispagebar:"N"}); // autocondition:"executives_id='"+executiveId+"'",querycontion:"executives_id='"+executiveId+"'"
}
...
function onSave(){
	...
	if ("Y" == ischeck) {
		if(!saveFlag){
			...
			var backValue = JSPFree.editTableCheckData(d1_A_BillCard, "Add", "法定代表人高管及重要关联人信息", "CRRS_SINGLE_EXECUTIVES","","2");
			...
			var backValue = JSPFree.editTableCheckData(d1_A_BillCard, "Edit", "法定代表人高管及重要关联人信息", "CRRS_SINGLE_EXECUTIVES","","2");
			if (backValue == "" || "undifind" == backValue) {
				return;
			} else if (backValue == "OK") {
		...
		if(saveFlag){
			...
			defaultValue ={executives_id:executive.executives_id,customer_type:"2",data_dt:executive.data_dt,issued_no:jso_OpenPars.issued_no};
