...
function AfterInit() {
	...
	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/group/crrs_group_group_client_CODE2.xml",null,{list_btns:"[icon-add]新增/insert1;[icon-edit]编辑/update1;[icon-remove]删除/delete1;[icon-p67]下载模板/downModel;[icon-p68]导入/importGYLFun;[icon-p67]导出/downSingleFun;[icon-p31]查看/onView"
		,querycontion:"type='2'",autocondition:"type='2'",list_ischeckstyle:"N",list_ismultisel:"N",ishavebillquery:"Y",autoquery:"N",isSwitchQuery:"N"});

	d1_BillList.pagerType="2";
	var _sql = "select * from crrs_group_group_client where type = '2' and "+ getQueryCondition(str_LoginUserOrgNo) +" order by data_dt desc";
...
function onQueryEvent(_condition) {	
	var _sql = "select * from crrs_group_group_client where type = '2' ";
 ...
 */
function importGYLFun(){
	JSPFree.openDialog("文件上传","/yujs/crrs/collection/importGYL.js", 500, 240, null,function(_rtdata){
		if (_rtdata == true) {
        	JSPFree.alert("上传成功");
...
function downModel(){
	...
	var src = v_context + "/crrs/common/downLoadModel?fileNm=23-SupplyChainFinanceInfo.xls";
 ...
 */
function insert1(){
	JSPFree.openDialog("基础信息","/yujs/crrs/collection/addGyl.js", 950,600, null,function(_rtdata){
 ...
 */
function update1(){
	var gylData = d1_BillList.datagrid('getSelected');
	if (gylData == null) {
		$.messager.alert('提示', '必须选择一条记录进行操作', 'warning');
		return;
	}
	JSPFree.openDialog2("基础信息","/yujs/crrs/collection/editGyl.js",950,600,{gylData:gylData},function(_rtdata){
...
function viewRule(){
	var jso_par1 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_GROUP_GROUP_CLIENT','crrs_group_members','crrs_ent_credit'," +
					"'crrs_ent_group_client') and ruletype='确定性校验'"};
	var jso_par2 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_GROUP_GROUP_CLIENT','crrs_group_members','crrs_ent_credit'," +
					"'crrs_ent_group_client') and ruletype='一致性校验'"};
	var jso_par3 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_GROUP_GROUP_CLIENT','crrs_group_members','crrs_ent_credit'," +
					"'crrs_ent_group_client') and ruletype='提示性校验'"};
...
function onView(){
	...
	JSPFree.openDialog2("基础数据","/yujs/crrs/dataquery/viewGyl.js",950,600,{gylData:gylData},true);
}
...
function delete1(){
	...
	JSPFree.confirm("提示", "你真的要删除选中的记录吗?", function(_isOK){
		if(_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.crrs.collection.service.CrrsCollectionBS", "deleteSupplyChainData",{rid:gylData.rid,customer_code:gylData.customer_code,data_dt:gylData.data_dt});
			if(jso_rt.msg == "OK"){
