...
function AfterInit() {
	str_templetcode = jso_OpenPars2.tempeltcode; // 模板名称
	...
	if ("1" == type) {
		result_tab_name = "CRRS_RESULT_SURE";
		if ("1" == source) {
			jso_CardConfig = {
		...
		result_tab_name = "CRRS_RESULT_CONSISTENCY";
		if ("1" == source) {
			jso_CardConfig = {
		...
		result_tab_name = "CRRS_RESULT_WARN";
		if ("1" == source) {
			jso_CardConfig = {
...
function onSave() {
	...
	var pkvalue = "";
	if (fromtable  == "crrs_group_executives" || fromtable  == "crrs_single_executives") { // 集团高管 或者 单一法人高管
		tablename_en = d1_A_BillCard.templetVO.templet_option.savetable;
		pkvalue = d1_A_BillCard.OldData.rid; 
	} else {
		tablename_en = d1_BillCard.templetVO.templet_option.savetable;
		pkvalue = d1_BillCard.OldData.rid; 
	}
