//列表购物车参照,即可以实现翻页多选.
var ary_cartkeys = ['problemcode', 'rule_name'];   //购物车的字段
...
function AfterInit(){
  JSPFree.createSplitByBtn("d1","左右",620,["确定/onConfirm","取消/onCancel"]);
	...
	var whereSql = '';
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.business.service.BfdGetReportBS", "getReportCNList", {reportType:reportType});
	if (jsn_result.code == 'success') {
		whereSql = jsn_result.data;
	}
  //创建左边的表
  JSPFree.createBillList("d1_A","/biapp-bfd/freexml/engineTask/bfd_engine_rule_task_ref.xml",null,{isSwitchQuery:"N", refWhereSQL: whereSql});
  ...
  //创建右边的购物车表
  JSPFree.createBillListByItems("d1_B","/biapp-bfd/freexml/engineTask/bfd_engine_rule_task_ref.xml",ary_cartkeys,ary_cartnames,ary_cartwidths);
...
function onConfirm(){
	...
	for(var j=0;j<jsy_datas.length;j++){
		var code = jsy_datas[j]['problemcode'];
