...
function AfterInit() {
    ...
    document.getElementById("d1").innerHTML =
        ...
        "<div class=\"easyui-panel\"  id=\"import-excel-template\" style=\"width:100%;heisht:100%;max-width:500px;max-height:450px;padding:30px 80px;\">" +
...
function AfterBodyLoad() {
    ...
    var result = JSPFree.doClassMethodCall("com.yusys.bfd.report.service.BfdImportReportDataFileService", "getRptOrgNoData");
...
function importReportData() {
    ...
    if (result.success == "ok") {
        ...
        $('#importFileForm').form('submit', {
            url: v_context + "/bfd/report/importReportDat?rptOrgNo=" + rptOrgNo + "&tabNameEn=" + tab_name_en,
            onSubmit: function () {
            },
...
function getCheckResult(flag) {
    ...
    if (file_typename != '.dat') {
        JSPFree.alert("请选择dat文件!");
        return;
    }
    var rptOrgNo = $("#rpt_org_no").combobox('getValue');
    if (!rptOrgNo) {
    ...
    var result = JSPFree.doClassMethodCall("com.yusys.bfd.report.service.BfdImportReportDataFileService", "importBeforeCheck", json);
...
function getImportProcess() {
    ...
    var result = JSPFree.doClassMethodCall("com.yusys.bfd.report.service.BfdImportReportDataFileService", "getImportProcess", json);
    var process = result.process;
    if (process) {
