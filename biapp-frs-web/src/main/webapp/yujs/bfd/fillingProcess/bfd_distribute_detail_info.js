...
function AfterInit(){
	...
	// 查看详情前，先确认表是否存在
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.business.service.BfdBusinessBS","checkTab",jso_pars);
	if (jso_rt.msg != "OK") {
	...
	var str_className = "Class:com.yusys.bfd.business.service.BfdModelTempletBuilderNoRpt.getTemplet('" + tabName + "','" + tabNameEn + "',''" + dataDt + ',' + "" + str_LoginUserOrgNo + "')";// 
...
function getSql() {
	var _sql = "";
	var col_arry1 = [];
	var jso_col = JSPFree.getHashVOs("select col_name_en from bfd_cr_col where tab_name='" + tabName + "' and is_pk='Y' and (col_name_en != 'IS_NULL' and col_name_en != 'IS_NOT_NULL') order by col_no");
	...
	var jso_col1 = JSPFree.getHashVOs("select col_name_en from bfd_cr_col where tab_name='" + tabName + "' and (col_name_en != 'IS_NULL' and col_name_en != 'IS_NOT_NULL') order by col_no");
	...
	var tabNameEnZ = "Z" + _date.substring(2, _date.length) + "_" + _tabNameEn;

	_sql = "select " + col_arry1.toString() + " from (select a.* from " + tabNameEn + " a,(select " + col_arry[0].toString() + " from " + tabNameEnZ
		+ " MINUS select " + col_arry[0].toString() + " from " + tabNameEn + "_R) b where 1=1";
	for (var j = 0; j < jso_col.length; j++) {
		_sql += " and a." + jso_col[j].col_name_en + "=b." + jso_col[j].col_name_en;
	}
	_sql += ") where 1=1 " + " and " + BfdFreeUtil.getDateColumn(tabName) + "='" + dataDt + "'";
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.common.service.BfdValidateQueryConditionBS", "getQueryConditionForBusi", {"_loginUserOrgNo": str_LoginUserOrgNo});
...
function selectDownOn() {
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "selectDataOn", {
		jsonData: json_data,
		taskId: taskId,
		tabNameEn: tabNameEn,
		deptNo: dept_no
...
function selectAllDownOn() {
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "selectAllDataOn", {
		sql: querySql,
		taskId: taskId,
		tabNameEn: tabNameEn,
		dataDt: dataDt,
...
function selectDownOff() {
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "selectDataOff", {jsonData:json_data,colArry:col_arry.toString(),tabNameEn:tabNameEn});
	if (jsn_result.msg == 'OK') {
...
function onSelect(_index,_title) {
	...
	if(newIndex==1){
		...
		if(!isLoadTabb_2){
			var str_className2 = "Class:com.yusys.bfd.business.service.BfdModelTempletBuilderForOrg.getTemplet('"+tabName+"','"+tabNameEn+"','"+str_LoginUserOrgNo+"','_R','"+taskId+"')";
