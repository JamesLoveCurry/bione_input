...
function AfterInit(){
	...
	var str_className2 = "Class:com.yusys.bfd.business.service.BfdModelTempletBuilderForAudit.getTemplet('"+tabName+"','"+tabNameEn+"','"+str_LoginUserOrgNo+"','_R','"+taskId+"','force_sub_sts')";
	JSPFree.createBillCard("d1_A", "/biapp-bfd/freexml/fillingProcess/bfd_filling_process_examin_detail.xml", null, null)
...
function onConfirm(){
	...
	JSPFree.confirm('提示', '你真的要通过该任务吗?', function(_isOK){
		if (_isOK){
			...
			var jso_col = JSPFree.getHashVOs("select col_name_en from bfd_cr_col where tab_name='"+tabName+"' and (col_name_en != 'IS_NULL' and col_name_en != 'IS_NOT_NULL') order by col_no");
			...
			maskUtil.mask();
			setTimeout(function () {
				var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "updateDataByTaskByRids1", jso_Pars);
				maskUtil.unmask();
...
function onBack(){
	JSPFree.confirm('提示', '你真的要退回该任务吗?', function(_isOK){
		if (_isOK){
			//弹出窗口,传入参数,然后接收返回值!
			JSPFree.openDialog("填写原因","/yujs/bfd/fillingProcess/bfd_distribute_detail_fillin_reason.js",600,300,{templetcode:"/biapp-bfd/freexml/fillingProcess/bfd_filling_process_remark.xml"},function(_rtdata){
				if(_rtdata != null && _rtdata != undefined && null != _rtdata.reason && undefined != _rtdata.reason){
					...
					maskUtil.mask();
					setTimeout(function () {
						var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "updateDataByTaskByRids2", jso_Pars);
						maskUtil.unmask();
						if (jsn_result.msg == 'OK') {
...
function onView(_this) {
	...
	JSPFree.openDialog2("查看详情","/yujs/bfd/fillingProcess/bfd_distribute_examine_detail.js",1000,600,jso_Pars,function(_rtdata) {
	});
}
