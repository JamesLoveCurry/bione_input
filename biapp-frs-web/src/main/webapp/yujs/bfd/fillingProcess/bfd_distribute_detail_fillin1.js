...
function AfterInit(){
	...
	var str_className1 = "Class:com.yusys.bfd.business.service.BfdModelTempletBuilderForDistribute.getTemplet('"+tabName+"','"+tabNameEn+"','"+str_LoginUserOrgNo+"','_R','"+taskId+"','0')";
	JSPFree.createBillList("d1_A_1",str_className1,null,{list_btns:"[icon-p41]编辑/updateDate(this);$VIEW;",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N"});
...
function onSelect(_index,_title) {
	...
	if(newIndex==1){
		...
		var str_className2 = "Class:com.yusys.bfd.business.service.BfdModelTempletBuilderForDistribute.getTemplet('"+tabName+"','"+tabNameEn+"','"+str_LoginUserOrgNo+"','_R','"+taskId+"','1')";
		JSPFree.createBillList("d1_A_2",str_className2,null,{list_btns:"[icon-p41]编辑/updateDate1(this);$VIEW;",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N"});
...
function updateDate(){
	...
	JSPFree.openDialog2("编辑","/yujs/bfd/fillingProcess/bfd_distribute_detail_fillin_edit.js",1100,600,jso_Pars,function(_rtData){
		if (_rtData != null) {
			if ("dirclose" == _rtData.type) {
				return;
			}
			if (_rtData == true) {
				if (json_data[0].data_ources == BfdFreeUtil.getDataOurces().etl_process){//etl才处理
					if (json_data[0].data_modify == BfdFreeUtil.getDataModefy().data_maintenance){//如果维护界面，则其减一
						var _par={tab_name_en:tabNameEn,data_dt:json_data[0].data_dt,number:-1,source_type:BfdFreeUtil.getSourceType().data_maintenance,rpt_org_no:json_data[0].rpt_org_no};
						JSPFree.doClassMethodCall("com.yusys.bfd.dataoperation.BfdStatisticalOperation","statisticalOperationForForm",_par)
					}
					if (json_data[0].data_modify != BfdFreeUtil.getDataModefy().data_filling){//本身不是填报界面才更新
						var _par={tab_name_en:tabNameEn,data_dt:json_data[0].data_dt,number:1,source_type:BfdFreeUtil.getSourceType().data_filling,rpt_org_no:json_data[0].rpt_org_no};
						JSPFree.doClassMethodCall("com.yusys.bfd.dataoperation.BfdStatisticalOperation","statisticalOperationForForm",_par)
					}
				}
...
function submission(){
	...
	// 修改数据状态：1：待审核
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "updateDataByTaskByRids", {allrids:jso_allrids,status:'1',type:"1",userNo:str_LoginUserCode});
...
function doUpdateBackStatus(billList,_rids,_reason){
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO","doResultBack",jso_par);
