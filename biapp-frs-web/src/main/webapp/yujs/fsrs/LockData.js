//锁定与解锁数据
function AfterInit(){
	JSPFree.createBillList("d1","/biapp-fsrs/freexml/fsrs/fsrs_lock_data_CODE1.xml",null,{list_btns:"[icon-p38]新增/createLock;[icon-p18]锁定/onLock_1;[icon-p20]解锁/onUnLock_1",isSwitchQuery:"N"});
...
//新增锁定
function createLock(){
	JSPFree.openDialog("新增","/yujs/fsrs/LockDataDate.js",350,350,{currSQL:d1_BillList.CurrSQL},function(_rtdata){
		if (_rtdata != null) {
...
function doCommitUpdateDB(data_dt, status){
	jso_rt = JSPFree.doClassMethodCall("com.yusys.fsrs.lock.service.FsrsLockBSDMO", "startLockData", {data_dt:data_dt,status:status});
