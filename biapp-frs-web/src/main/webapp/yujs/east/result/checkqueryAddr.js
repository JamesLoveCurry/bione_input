//初始化界面,按地区查询,菜单配置路径是【/frs/yufreejs?js=/yujs/east/checkqueryAddr.js】
var org_no = "";
var org_class = "";
var dataDt = null;
var whereSql = '';
...
function AfterInit() {
	...
	var jso_org = JSPFree.getHashVOs("SELECT org_no,org_class,addr FROM rpt_org_info where org_type='04'  and mgr_org_no = '"+str_LoginUserOrgNo+"'");
	...
	if (addr!=null && addr!="") {
		JSPFree.createBillList("d1_1", "/biapp-east/freexml/east/result/east_cr_summer_data_addr.xml",null,{isSwitchQuery:"N",autoquery:"N",list_ispagebar:"Y"}); // 按表查询
		...
		JSPFree.createBillList("d1_2", "/biapp-east/freexml/east/result/east_cr_summer_QueryByRule_addr.xml",null,{isSwitchQuery:"N",autoquery:"N",list_ispagebar:"Y"}); // 按规则查询
...
function getSql(str_data_dt) {
	...
	if (str_data_dt == "" || str_data_dt == null) {
		// 获取最新一期的日期
		var jso_data = JSPFree.getHashVOs("SELECT max(data_dt) data_dt FROM v_east_cr_summary_addr");
	...
	var _sql ="SELECT tab_name, SUM (sum_count) sum_count, SUM (fail_count) fail_count, " +
		...
		"FROM (SELECT tab_name, sum_count, fail_count, data_dt FROM V_EAST_CR_SUMMARY_ADDR WHERE 1 = 1";
	...
	_sql = _sql + ") where 1=1";
	_sql = _sql + " and data_dt='" + str_data_dt +"'";
...
function getSql1(str_data_dt) {
	...
	if (str_data_dt == "" || str_data_dt == null) {
		// 获取最新一期的日期
		var jso_data = JSPFree.getHashVOs("SELECT max(data_dt) data_dt FROM v_east_cr_summary_rule_addr");
		if(jso_data != null && jso_data.length > 0){
			str_data_dt = jso_data[0].data_dt;
		}
	}
	...
	"from ( " +
	"select s.data_dt,s.rule_id,s.report_no,s.rule_name,s.tab_name,s.col_name,s.type_cd,s.disp_cd,s.weight,s.sum_count,s.fail_count,s.create_tm,s.rid,s.addr,r.rule_source " +
	"from v_east_cr_summary_rule_addr s left join east_cr_rule r on s.rule_id = r.id WHERE 1 = 1";
...
function onErrorSummary(_condition){
	var _sql ="SELECT tab_name, SUM (sum_count) sum_count, SUM (fail_count) fail_count, " +
			"CASE WHEN SUM (sum_count) = 0 THEN '' ELSE TO_CHAR (ROUND (SUM (fail_count) / SUM (sum_count), 4) * 100, 'FM999990.00') || '%' END AS fail_rate,data_dt " +
			"FROM (SELECT tab_name, sum_count, fail_count, data_dt, addr FROM V_EAST_CR_SUMMARY_ADDR WHERE 1 = 1";
	_sql += " and " + whereSql;
	_sql = _sql + ") where 1=1";

	if(_condition!=""){
		_condition = _condition.replace('addr_ot', 'addr').replace(/%/g,"").replace(/like/g, '=');
		_sql += "and" + _condition;
	}
	_sql += " group by tab_name, data_dt order by fail_count desc";
...
function onErrorSummary1(_condition){
	...
	if(_condition!=""){
		if(_condition!=""){
			_condition = _condition.replace('addr_ot1', 'addr').replace(/%/g,"").replace(/like/g, '=');
			_sql += "and" + _condition;
		}
	}
	_sql += " group by data_dt, rule_id, report_no, rule_name, tab_name, col_name,type_cd,disp_cd ";
...
function viewFailDetailByTab(_btn) {
	var _addr_ot = JSPFree.getBillQueryFormValue(d1_1_BillQuery).addr_ot;

	var _addr_ot1 = "";
	var org_type = "";
	// 判断是否存在报送机构查询框，如果存在，说明是报送机构，如果不存在，说明是非报送机构
	if (_addr_ot != null && _addr_ot != "") {
		// 根据中文表名，找对应的英文code
		var jso_org = JSPFree.getHashVOs("SELECT addr FROM rpt_org_info where org_type='04'  and addr = '"+_addr_ot+"'");
		if(jso_org != null && jso_org.length > 0){
			_addr_ot1 = jso_org[0].addr;
		}
		org_type = "Y";
	} else {
		_addr_ot1 = addr;
	...
	var jso_OpenPars = {tabName: row.tab_name,tabNameEn:row.tab_name_en,dataDt:row.data_dt,orgNo:org_no1,org_type:org_type,addr:addr};
	// 查看详情前，先确认表是否存在
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.east.checkresult.summary.service.FailDetailBS", 
			"checkTab",jso_OpenPars);
	
	if (jso_rt.msg == "OK") {
		JSPFree.openDialog("错误明细","/yujs/east/result/failDetailAddr.js", 950, 500, jso_OpenPars,function(_rtdata){});
	} else {
...
function exportFialDetailByTab(_btn){
	...
	var sql = " select * from " + errorTableName + " where address = '" + addr_ot1 + "'";
	...
	var param = {tabName: row.tab_name,
		...
		queryType:'2',
		detailType: "EAST", //表类型
		downloadType: "6" ,  //导出类型：1、维护导出，2、法人错误明细导出，3、填报错误导出，4、填报全量导出，5、已修改记录导出,6 地区错误明细导出
		fileType: "EXCEL",
		params: 'address=' + addr_ot1 + ' and cjrq=' + data_dt
	};
...
function viewFailDetailByRule(_btn) {
	var _addr_ot = JSPFree.getBillQueryFormValue(d1_2_BillQuery).addr_ot1;
	...
	if (_addr_ot != null && _addr_ot != "") {
		// 根据中文表名，找对应的英文code
		var jso_org = JSPFree.getHashVOs("SELECT org_no,addr FROM rpt_org_info where org_type='04' and addr = '" + _addr_ot + "'");
		if (jso_org != null && jso_org.length > 0) {
	...
	if (disp_cd.indexOf("常规") != -1) {
		...
		if (jso_rt.msg == "OK") {
			JSPFree.openDialog("错误明细", "/yujs/east/result/failDetailByRuleAddr.js", 950, 600, jso_OpenPars, function (_rtdata) {
			});
		} else {
			JSPFree.alert("当前明细表不存在，无法查看明细数据!");
		}
...
function historyByRule(_btn){
	var addr = JSPFree.getBillQueryFormValue(d1_2_BillQuery).addr_ot1;
	var dataset = _btn.dataset;
	var index = dataset.rowindex;
	var rows = d1_2_BillList.datagrid("getRows");
	var row = rows[index];//index为行号
	...
	jso_OpenPars.addr = addr;
	JSPFree.openDialog("历史数据","/yujs/east/result/checkresultRuleHistoryAddr.js", 700, 450, jso_OpenPars,function(_rtdata){});
}
