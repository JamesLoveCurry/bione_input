...
JSPFree.createBillList("d1_1_A","east/engineTask/east_cr_task_for_log_CODE",null,{isSwitchQuery:"N"});  //第1个页签上边
JSPFree.createBillList("d1_1_B","east/engineTask/east_cr_log_task_CODE2");  //第1个页签右边的下边
JSPFree.createBillList("d1_2","east/engineTask/east_cr_log_task_CODE",null,{isSwitchQuery:"N"});
...
function download1() {
	...
	var src = v_context + "/east/engine/log/download?taskId="
			+ task_id;
...
//出报送数据
function download2() {
	// 判断grid是否有数据，无数据提示不能导出
	if (d1_2_BillList.datagrid('getData').total == 0) {
		$.messager.alert('提示', '当前无数据，无法导出！', 'warning');
		return;
	}
	...
	var src = v_context + "/east/engine/log/downloadlog?taskName="
			+ task_name
			+ "&dataDt="
			+ data_dt
			+ "&taskType="
