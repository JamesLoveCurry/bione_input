...
function AfterInit() {
    ...
    document.getElementById("d1").innerHTML =
        ...
        "<input name=\"dataDt\" value=\"" + dataDt + "\" type=\"hidden\"> " +
        "<input name=\"orgNo\" value=\"" + orgNo + "\" type=\"hidden\"> " +
...
function add() {
    ...
    if (file_typename.toLocaleLowerCase() != '.xml') {
        $.messager.alert('提示', '文件格式错误，只能上传xml文件！', 'warning');
        return;
    }
    ...
    $('#importFileForm').form('submit', {
        url: v_context + "/east/data/upload/receipt",
        onSubmit: function () {
        ...
        success: function (result) {
            if ("success" == result) {
                ...
                JSPFree.closeDialog({title: 'error', msg: "回执导入失败"});
            }
        }
    });
}
