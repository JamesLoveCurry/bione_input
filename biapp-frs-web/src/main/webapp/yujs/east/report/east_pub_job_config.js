...
function AfterInit(){
	...
	JSPFree.createBillList("d1_A", "/biapp-east/freexml/east/report/east_pub_job_CODE.xml",null,{isSwitchQuery:"N"});
 	JSPFree.createBillList("d1_B", "/biapp-east/freexml/east/report/east_pub_job_b_CODE.xml",null,{isSwitchQuery:"N"});
...
function startJob(){
	...
	var jso_data = JSPFree.doClassMethodCall("com.yusys.east.scheduled.EastJobBS","startJob",jso_par); 
	if(jso_data.code == 0){
...
function endJob(){
	...
	var jso_par = {jobName:selectDatas[0].name};
	var jso_data = JSPFree.doClassMethodCall("com.yusys.east.scheduled.EastJobBS","stopJob",jso_par); 
	if(jso_data.code == 0){
...
function delete1(){
	...
	//校验是否被使用了
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.east.scheduled.EastJobBS", "checkDeleteJob",{jobId:job.rid});
	if(jso_rt.code == 1){
	...
	JSPFree.confirm("提示", "你真的要删除选中的记录吗?", function(_isOK){
		if(_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.east.scheduled.EastJobBS", "deleteJob",{jobId:job.rid});
			if(jso_rt.code == 0){
