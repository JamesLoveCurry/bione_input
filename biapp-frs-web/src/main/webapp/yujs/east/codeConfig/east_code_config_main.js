...
$(function(){
    $('#dg').datagrid({
        ...
        url:v_context + "/east/code/getAllPraCode",//向后台请求数据
        //data: datagird_data,
        singleSelect:"true" ,
        fitColumns:"true",
        striped:true,
        ...
        onExpandRow: function(index,row){
            $('#ddv-'+index).datagrid({
                //view: detailview,//detailview必须写
                url:v_context +'/east/code/getByCodeType?codeType='+row.codeType,//点击后再去后台请求数据
...
function exportRule() {
    ...
    var src = v_context + "/east/code/export?codeTypeName=" + codeTypeName;
...
function importRule() {
    JSPFree.confirm("提示","码值导入会全量更新系统中的码值字典，请谨慎操作，请确认是否执行导入操作！",function(_isOK){
        if(_isOK){JSPFree.openDialog("文件上传", "/yujs/east/codeConfig/east_input_code.js", 500, 240, null,function(_rtdata) {
