...
<script type="text/javascript">
	...
	function initTree() {
		$.ajax({
			cache : false,
			async : false,
			url : '${ctx}/rpt/input/catalog/' + dirType + '/list.json?d='+new Date(),
			success : function(result) {
				...
				var dirId = parent.document.getElementById("catalogId");
				if(dirId!=null && dirId.value!=null && dirId.value!=''){
					$(result).each(function() {
						//如果该目录Id存在目录表中，则选择。
						if(dirId.value==this.id){
							id = dirId.value; 
						}
					});
				}
	...
	function getCurrentInfo(node) { //load current info
		...
		$.ajax({
			...
			error : function() {
				$("#mainform input[name='upCatalog']").val("0");
				$("#mainform input[name='upCatalog']").val(rootName);
			}
		});
		$(".l-dialog-btn").remove();
				...
				$.ligerDialog.confirm('确实要删除该记录吗?', function(yes) {
					if (yes) {
						...
						$.ajax({
							...
							cache : false,
							type : "POST",
							url : "${ctx}/rpt/input/catalog/" + currentNode.id+"?d="+new Date(),
							dataType : "script",
							success : function() {
								flag = true;
							}
	...
	function savedCallback() {
		...
		if(nodes.length !=0&&nodes[0].upId==$('#upCatalog').val()&&dirId!=nodes[0].id){
			BIONE.tip('已存在同名节点！');
			return ;
		}
	...
	function dirIdCallback(){
		var dirId = document.getElementById('catalogId').value;
		var dirName = document.getElementById('catalogName').value;
		
		try {
			window.parent.dictCatalogCombox._changeValue(dirId, dirName);
		} catch(e) {
			try {
				parent.selectCata(dirName, dirId);
			} catch(e) {
			}
		} finally {
			BIONE.closeDialog("dirList");
		}
	}
	//CANCLE
	...
	function addDir() {
		...
		if (currentNode) {
			...
			$("#mainform input[name=parentName]").val("补录字典目录");
			$("#mainform input[name=upCatalog]").val("0");
		}
