...
<script type="text/javascript">
	...
	function  addIdxCatalog(){
		...
		var modelDialogUri = "${ctx}/rpt/input/task/newCatalog?upNo=" + upNo+"&upName="+encodeURI(encodeURI(upName));
		BIONE.commonOpenDialog("任务目录添加", "rptIdxInputEditCatalog","660","320",modelDialogUri);		 
	}
    function  updateIdxCatalog(){
		...
		var modelDialogUri = "${ctx}/rpt/input/task/initUpdateCatalog?catalogId=" + id+"&upName="+ encodeURI(encodeURI(upName));
		BIONE.commonOpenDialog("任务目录修改", "rptIdxInputEditCatalog","660","320",modelDialogUri);
    }
    function deleteIdxCatalog() {
		...
		$.ajax({
			type : "POST",
			url : '${ctx}/rpt/input/task/canDeleteCatalog.json?catalogId=' + currentNode.id,
			success : function(result) {
				if (result == "1") {
					BIONE.tip("该目录下面包含任务，不能进行删除操作！");
					return;
				} else if (result == "0") {
					$.ligerDialog.confirm('将执行级联删除操作，是否继续', '指标目录删除', function(yes) {
						if (yes) {
							if (currentNode.id != null) {
								BIONE.ajax({
									type : 'POST',
									url : '${ctx}/rpt/input/task/delCatalog/' + currentNode.id
								}, function(result) {
									var returnStr = result.msg;
									if (returnStr == "0") {
										leftTreeObj.reAsyncChildNodes(currentNode.getParentNode(), "refresh");
	...
	function initTree() {
		var  url_ =  "${ctx}/rpt/input/task/getTaskTree.json";
		var setting ={
				async:{
					enable:true,
					url:url_,
	...
	function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		var  nodeType = currentNode.params.nodeType;
        initCurrentTab(currentNode.id,nodeType,currentNode.text);
	} 	
	function  initCurrentTab(nodeId,nodeType,nodeName){
		var curTabUri = "${ctx}/rpt/input/task/taskInfos?nodeId="+nodeId;
		var tabTitleName = "任务节点"; 
		if(nodeType=="taskinfo")
		{
			tabTitleName = "任务"; 
			curTabUri = "${ctx}/rpt/input/task/taskInfoFrame?taskId="+encodeURI(encodeURI(nodeId));
		}
		curTabUri = curTabUri+"&nodeType="+nodeType+"&previewObjId="+previewObjId+"&canOper="+canOper+"&d="+new Date().getTime();
		tabTitleName =tabTitleName+"["+nodeName+"]";
		tabObj.removeTabItem("rptIdxInputTab");
		tabObj
			.addTabItem({
				tabid : "rptIdxInputTab",
				text:tabTitleName,
				showClose : false,
				content : '<iframe frameborder="0" id="idxInputTabFrame" name="idxInputTabFrame" style="height:'+height+'px;" src='+curTabUri+' ></iframe>'
			});
			tabObj.selectTabItem("rptIdxInputTab");	    
	}
	function   reload(){
		searchHandler();
	}
