...
<script type="text/javascript">
	...
	$(function() {
		...
		//加载数据
		loadTree("${ctx}/bione/admin/logicSys/getResList.json?logicSysNo="+id,leftTreeObj);
		
		rightTreeObj = $.fn.zTree.init($("#rightTree"),setting2);
		//加载数据
		loadTree("${ctx}/bione/admin/logicSys/getAuthResList.json?logicSysNo="+id,rightTreeObj);
		...
		BIONE.createButton({
			...
			click : function() {
				...
				var url = "${ctx}/bione/admin/logicSys/saveAuthRes.json?logicSysNo="+id+"&params="+paramStr;
				f_admin_save(url);
			}
		});
	...
	//删除节点
	function treeRemoveNode(event, treeId, treeNode) {
		...
		if(nodes.length>0){
			for(var i=0;i<nodes.length;i++ ){
				...
				$.ajax({
					cache : false,
					async : false,
					url : "${ctx}/bione/admin/logicSys/checkRes",
					dataType : 'json',
	...
	function f_admin_save(url){
		$.ajax({
			...
			type : "get",
			success : function() {
				BIONE.closeDialog("authResWin","保存成功！");
			},
			error : function(result, b) {
				BIONE.tip('发现系统错误 <BR>错误码：' + result.status);
			}
...
<body>
	...
	<div id="template.left.up">未选择资源</div>
	<div id="template.right.up">已选择资源</div>
</body>
</html>