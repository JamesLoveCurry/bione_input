...
<script type="text/javascript">
	...
	$(function() {
		...
		grid = $("#maingrid").ligerGrid({
			...
			url : "${ctx}/bione/admin/orggrp/list",
			usePager : true, //服务器分页
			sortName : 'grp.create_time',//第一次默认排序的字段
			sortOrder : 'asc', //排序的方式
			pageParmName : 'page',
	...
	function create(){
		BIONE.commonOpenIconDialog("机构组信息录入", "new_set","${ctx}/bione/admin/orggrp/newset", 'new_set');
	};
	...
	function edit(){
		...
		if(rows.length==1){
			...
			BIONE.commonOpenIconDialog("机构组信息修改", "new_set","${ctx}/bione/admin/orggrp/editset?groupId="+groupId, 'edit_set');
		} else {
			BIONE.tip("请选择一条记录");
		}
	};
			...
			$.ligerDialog.confirm("您确定删除记录吗?",function(data){
				if(data){
					$.ajax({
						cache : false,
						async : true,
						url : "${ctx}/bione/admin/orggrp/delgroup?groupId="+ids,
						type : 'POST',
	...
	function config(){
		...
		if(rows.length==1){
			var groupId=rows[0].grpId;
			if(${isSuperUser}){
				BIONE.commonOpenDialog("配置机构信息","config_obj",450,500,"${ctx}/bione/admin/orggrp/config?grpId="+groupId,"config_obj");
			} else {
