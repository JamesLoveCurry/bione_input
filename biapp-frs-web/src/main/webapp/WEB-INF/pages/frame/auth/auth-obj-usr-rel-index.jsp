...
<script type="text/javascript">
				...
				.createButton({
					...
					click : function() {
						if (navtab) {
							var objIds = "";
							var leftSelNode;
							currentTabId = window['navtab']
									.getSelectedTabItemID();
							if (eval(currentTabId + "_tree")) {
								var leftSelNodes = eval(currentTabId + "_tree")
										.getSelectedNodes();
							...
							if (leftTree) {
								var checkedNodes = leftTree
										.getCheckedNodes(true);
								var checkedNodesArray = leftTree
										.transformToArray(checkedNodes);
								...
								$
										.ajax({
											async : true,
											cache : false,
											url : "${ctx}/bione/admin/authObjUsrRel/saveObjUserRel",
											dataType : 'json',
											type : "post",
											data : {
												relObjs : JSON2
	...
	function initUserTree(searchText) {
		$.ajax({
			cache : false,
			async : true,
			url : "${ctx}/bione/admin/authObjUsrRel/getUserTree.json",
			dataType : 'json',
			type : "post",
			data : {
