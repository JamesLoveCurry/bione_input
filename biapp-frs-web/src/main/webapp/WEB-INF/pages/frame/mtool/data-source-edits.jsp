...
<script type="text/javascript">
	...
	$(function() {
		...
		mainform.ligerForm({
			fields : [ {
				...
				options : {
					resize:false,
					textField : 'driverType',
					valueField : 'driverId',
					onSelected : function(value) {
						if(!("${id}")){
							getUrl(value);
						}
						//oracle集群可能设计到多ip配置，所以放开connUrl字段，支持直接配置url
 						if(0 == value){
							$("#connUrl").parent().parent().parent().parent().show();
						}else{
							$("#connUrl").parent().parent().parent().parent().hide();
						}
					} 
				}
			}, {
	...
	$('#host, #port, #dbname').bind('blur', function() {
...
<body>
	<div id="template.center">
		<h3 id="batchRemind" style="color:red;">当配置Oracle-Rac多个ip时，可以直接把完整URL填写到连接URL字段中</h3>
		<form id="mainform" action="${ctx}/bione/mtool/datasource"
			method="post"></form>
	</div>
</body>
</html>