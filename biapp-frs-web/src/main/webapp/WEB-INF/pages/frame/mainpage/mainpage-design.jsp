...
<script type="text/javascript">
	...
	function saveFuc(){
		...
		$.ajax({
			cache : false,
			async : true,
			url : "${ctx}/bione/mainpage/saveLayout.json",
			dataType : 'json',
			type : "post",
			data : paramDatas,
			success : function(){
	...
	// 撤销布局方法
	function cancelFuc(){
		$.ligerDialog.confirm("即将清除您最近一次保存的个人布局，且不可恢复，是否继续?",function(flag){
			if(flag){
				$.ajax({
					cache : false,
					async : true,
					url : "${ctx}/bione/mainpage/cancelLayout.json",
					dataType : 'json',
					type : "post",
					data : {
						designId:designId
	...
	function initUserDesign(){
		$.ajax({
			cache : false,
			async : true,
			url : "${ctx}/bione/mainpage/initUserDesign.json",
			dataType : 'json',
			type : "post",
