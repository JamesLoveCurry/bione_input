...
<script type="text/javascript">
	...
	$(function() {
		...
		var selNodes;
		if(window.parent.taskTree){
			selNodes = window.parent.taskTree.getSelectedNodes();
		}
		...
		if("${id}") {
			...
			if(selNodes != null 
				...
				$("#mainform input[name=upCatalogId]").val(selNodes[0].getParentNode().params.catalogId);
			}else{
				// 新增根节点目录
			...
			if(selNodes != null 
				...
				$("#mainform input[name=upCatalogId]").val(selNodes[0].params.catalogId);
			}else{
				// 新增根节点目录
				$("#mainform input[name=upCatalogNm]").val("无");
				$("#mainform input[name=upCatalogId]").val(tree_root_id);
