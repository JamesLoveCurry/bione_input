...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#maingrid").ligerGrid({
			...
			method : 'post',
			url : '${ctx}/frs/pisamessage/taskConfigList',
	...
	//生成报文
	function msg_create() {
		BIONE.commonOpenDialog('生成报文', 'createXmlWin', '350', '350','${ctx}/frs/pisamessage/createXml');
	}
	...
	function msg_download() {
		...
		var src = "${ctx}/frs/pisamessage/downLoadFile?id="+rows[0].taskNo+"&d="+ new Date();
			...
			$.ligerDialog.confirm('您确定删除这' + ids.length + "条记录吗？", function(yes) {
				if (yes) {
					$.ajax({
						async : false,
						type : "post",
						url : '${ctx}/frs/pisamessage/deletePisaTask?&ids=' + idsStr,
