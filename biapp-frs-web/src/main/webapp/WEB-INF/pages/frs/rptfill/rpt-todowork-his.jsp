...
(function($) {
	...
	FinishStsRender = function(rowdata) {
			if (rowdata.sts == '0') { return "未提交";} else if (rowdata.sts == '1') { return "待复核";} else if (rowdata.sts == '2') { return "待审核";} else if (rowdata.sts == '3') { return "流程完成";}  else if (rowdata.sts == '4') { return "已完成";}else if (rowdata.sts == '10') { return "已驳回";}else { return rowdata.sts;}
	...
	// add by wusb at 20161207 添加零值校验状态渲染
	...
	//modify by wusb at 20161207 修改传参的顺序
	...
	GridObject = {
			...
			"isUpt" : {display : '任务状态', name : 'isUpt', minWidth : 50, width : 60, render : UpdateStsRender},
	...
	var SearchFormObject = {
			...
			"finishSts" : {display : "报表状态", name : "finishSts", newline : false, type : "select",width : '140', cssClass : "field", labelWidth : '90',
						...
						options : { data : [ { text : '未提交', id : "0"}, { text : '待复核', id : "1"}, { text : '待审核', id : "2"},{ text : '流程完成', id : "3"},{ text : '已驳回', id : "10"} ], cancelable  : true}
					},
			"updateSts" : {display : "任务状态", name : "updateSts", newline : false, type : "select",width : '140', cssClass : "field", labelWidth : '90',
			...
			"lineNm" : {display : "条线名称", name : "lineNm", newline : false, type : "select",width : '140', cssClass : "field", labelWidth : '90',
						comboboxName : "lineNm_sel", attr : { op : "=", field : "li.lineId"}
					},
			"oprNm" : {display : "操作用户", name : "oprNm", newline : false, type : "select",width : '140', cssClass : "field", labelWidth : '90',
					comboboxName : "oprNm_sel", attr : { op : "=", field : "li.lineId"}
					},
			"beginDate" : {display : "开始日期", name : "beginDate", newline : false, type : "date",width : '140', cssClass : "field", labelWidth : '90',
						attr : { op : "=", field : "i.begin_date"},
						options : { format : "yyyyMMdd"}
					},
			"endDate" : {display : "结束日期", name : "endDate", newline : false, type : "date",width : '140', cssClass : "field", labelWidth : '90',
...
//			check:{ enable : true, chkboxType :{"Y":"","N":""}, chkStyle :"checkbox"},
	...
	TaskFillGrid = function(gridUrl, columnSource, isOnDbClick, callBack, checkbox , delayLoad ,sortName,operType) {
		...
		var eles = {height : '99%', width : '100%',
					...
					method : 'get', url : gridUrl, sortName : sortName?sortName:'i.data_date, i.exe_obj_id, i.task_obj_id, i.task_id', 
		...
		grid.setHeight($("#center").height() - 170);
	};
	//SearchForm
...
<script type="text/javascript">
	$(function (){
		tmp.searchForm();
		TaskFillGrid(tmp.gridUrl, tmp.columns, false,null,false,false);
		tmp.initButtons();
	...
	var fileName = '';
	var tmp = {
			gridUrl : "${ctx}/rpt/frs/rptfill/flTaskList?orgTypes=${moduleType}&operType=${operType}",
			taskComBoBoxUrl : "${ctx}/frs/rptfill/reject/taskNmComBoBox?orgTypes=${moduleType}&flag=1",
			rptComBoBoxUrl : "${ctx}/frs/rptfill/reject/rptNmComBoBox",
			orgTreeSkipUrl : "${ctx}/frs/integratedquery/rptquery/searchOrgSetInfo?",
			columns : ["taskNm", "dataDate", "endTime", "taskObjNm", "exeObjNm", "finishSts", "sumpartRs", "logicRs", "warnRs", "zeroRs", "taskId"],
			...
			getMaxTskDatadate : function () {
				$.ajax({
					...
					error:function(){ BIONE.tip("任务实例最大日期获取异常，请联系系统管理员");}
				});
			},
			oper_view : function(){
				...
				var row = rows[0];
				BIONE.commonOpenDialog(row.taskObjNm+"-"+row.exeObjNm+"-"+row.dataDate, "submitInputWin", 800, 500, "${ctx}/rpt/frs/rptfill/getDetail?taskInstanceId="+row.taskInstanceId + "&orgNo=" + row.exeObjId + "&orgType=" + row.taskType + "&dataDate=" + row.dataDate + "&taskId=" + row.taskId, null);
			},
			initButtons : function () {
				var btns = [ 
				             { text : '查看', click : tmp.oper_view, icon : 'fa-clone', operNo : 'oper_fill'}]
								...
								click : function() {
									...
									$(":input[ltype=combobox]", form).each(
													function() {
														...
														grid.removeParm("sumpartRsSts");
														grid.removeParm("warnRsSts");
														grid.removeParm("zeroRsSts");//add by wusb at 20161207 添加零值校验
													});
