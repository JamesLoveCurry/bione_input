...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#maingrid").ligerGrid({
			...
			method : 'post',
			url : '${ctx}/frs/nrtmessage/acctcontrollList',
	...
	function oper_add() {
		BIONE.commonOpenLargeDialog('新增信息', 'contrFlowEidtWin','${ctx}/frs/nrtmessage/contrFlowEidt');
	}
	//修改属性
	function oper_modify() {
		...
		BIONE.commonOpenLargeDialog("修改信息", "contrFlowEidtWin","${ctx}/frs/nrtmessage/contrFlowEidt?id="+rows[0].recordId);
	}
	//批量删除报文集
			...
			$.ligerDialog.confirm('您确定删除这' + ids.length + "条记录吗？", function(yes) {
				if (yes) {
					$.ajax({
						async : false,
						type : "post",
						url : '${ctx}/frs/nrtmessage/deleteContrFlow?&ids=' + idsStr,
						success : function(res){
							BIONE.tip('删除成功');
							grid.loadData();
						},
