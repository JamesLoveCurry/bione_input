...
<script>
	(function($) {
		...
		//获取报表状态字典值
		 $.ajax({
				...
				success : function(result) {
					if(result){
						rptSts = result;
						//result    {0: "未归档", 1: "待复核", 2: "待审核", 3: "已归档", 9: "已冻结"}
		...
		onShowRptRsView = function(rptId, dataDate, taskType,
			...
			BIONE.commonOpenLargeDialog('校验结果查看',
					'checkResultWin', url, function() {
					});
		};
								...
								BIONE
										.ajax(
												...
												function(result) {
													...
													if (sts == "0"
															&& rptOperType == "03") {//未归档报表
														//modify by wusb at 20161118  待定。。。
														if(templateType && templateType == "01"){
															height = height - 30;
														}
														var url = ctx
																+ "/rpt/frs/rptfill/showTabPoc?path=rptfill-todowork-oper-poc&type="
																+ taskType
																+ "&taskType="
																+ taskType;
...
<script type="text/javascript">
	...
	var tmp = {
		...
		orgTreeSkipUrl : "${ctx}/frs/integratedquery/rptquery/searchOrgSetInfo",
	 	columns : ["taskNm", "dataDate", "taskObjNm", "exeObjNm", "taskId"],
		fields:["taskNm", "rptNm", "orgNm", "dataDate"],
		...
		list_down : function() {
			...
			var src = "${ctx}/rpt/frs/rptfill/flTaskList?orgTypes=${moduleType}&operType=${operType}&rptOperType=${rptOperType}&isListDownload=1&otherCondition="
					...
					success : function(result) {
						if (result.result) {
							if ("OK" == result.result) {
								grid.loadData();
								BIONE.tip("校验完成");
							} else if ("LASTDEPT" == result.result) {
								BIONE.tip("最末级的机构不可进行总分校验");
							} else {
						...
						if (rows[i].sts == '0') {//未归档
							if(flag == "force"){//强制提交
								...
								if ((rows[i].logicRs == null || rows[i].logicRs == '04' || rows[i].logicRs == '06')){
									...
									var logicFlag = tmp.getLogicValidResult(rows[i].templateId,rows[i].dataDate,rows[i].exeObjId);
									if(logicFlag){
									...
									success : function(result) {
										...
										if (result.result == "YES" && result.ins) {
											...
											BIONE.commonOpenDialog(
												...
												width,
												height,
												"${ctx}/rpt/frs/rptfill/childIns?sts=0&params="
														+ JSON2.stringify(params)
														+ "&taskInsIds="
														+ taskInsIds.join(",")
														+ "&orgNo="
														+ simpleOrg
											...
											if (data
												...
												$.ajax({
													cache : false,
													async : true,
													url : "${ctx}/rpt/frs/rptfill/submitTaskBatch?queryType=fuhe&moduleType=${moduleType}&rptOperType=03&taskFillOperType=03&taskInsIds="
															+ taskInsIds.join(",")
															+ "&authUsers="
															+ data.join(",")
															+ "&org="
												...
												$.ajax({
													cache : false,
													async : true,
													url : "${ctx}/rpt/frs/rptfill/submitTaskBatch?rptOperType=03&ignore=Y&taskFillOperType=03&taskInsIds="
															+ taskInsIds.join(","),
													dataType : 'json',
													type : "get",
													success : function() {
						...
						success : function(result) {
							...
							if (result.result == "YES" && result.ins) {
								...
								BIONE.commonOpenDialog(
									...
									width,
									height,
									"${ctx}/rpt/frs/rptfill/showTab?path=rptfill-org-child-ins&sts=0&params="
											+ JSON2 .stringify(params)
											+ "&taskInsIds="
											+ taskInsIds.join(",")
											+ "&orgNo="
		...
		importData : function() {
			...
			if (rows.length == 0) {
				...
				if (submitName && submitName != null && submitName != "") {
					BIONE.tip("任务[" + submitName + "]不是未归档状态,无法修改或导入");
					return;
				}
				BIONE.commonOpenDialog('导入数据文件', 'uploadWin', 600, 330,
						"${ctx}/rpt/frs/rptfill/showTab?path=rpt-todowork-uploadfile&dataDate="
		...
		oper_quote_rptCalculation : function() {
			...
			if (rows.length > 0) {
				for (var i = 0; i < rows.length; i++) {
					rptCalculation(rows[i].taskObjId, rows[i].dataDate, rows[i].exeObjId, rows[i].taskInstanceId, rows[i].taskType, rows[i].templateId);
				}
			}
		}
	};
	...
	//报表计算(只计算表间取数)
	function rptCalculation(rptId, dataDate, orgNo, taskInsId, operType, rptTmpId) {
		$.ajax({
			async : true,
