...
<script type="text/javascript">
		...
		$("#center").prepend(
				'<div id="tipMainDiv"'
					...
					+'<div id="tipContainerDiv"'
					   ...
					   +'<div id="tipAreaDiv">'
							+'tips : 1、数据日期与机构编号为指标共有维度，必选且不可更改&nbsp&nbsp&nbsp&nbsp&nbsp 2、下方维度树展示对应逻辑校验公式中所有指标的共有维度<br />'
						+'</div>'
					+'</div>'
				+'</div>');
		//初始化tip
	...
	//保存
	function f_save() {
		window.parent.validObj.saveData();
		tree_save();
	}
	
	function initTree(){
		var url = "${ctx}/report/frame/rptvalid/logic/listDimTree.json";
		var data = {
				'checkId' : checkId,
				'indexNos' : indexNos
				   };
	...
	function tree_save(){
		...
		var _url="${ctx}/report/frame/rptvalid/logic/selectDim";
		if(ids != "" && dimTypes != ""){
			$.ajax({
				async:true,
				type:"POST",
