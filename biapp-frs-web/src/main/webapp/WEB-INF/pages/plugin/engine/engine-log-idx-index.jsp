...
<script type="text/javascript">
	...
	function initSearchForm() {
		$("#search").ligerForm({
			fields : [{
				...
				options : {
					url : "${ctx}/report/frame/enginelog/idx/exeStsList.json"
				}
			} ]
		});
	};
	function initGrid() {
		grid = $("#maingrid").ligerGrid({
			...
			columns : [{
				...
				render : function(row,index,val){
					return "<a style='color:blue' onclick='f_open_view(\"" + row.id.taskNo + "\",\"" 
							+ row.id.dataDate + "\",\"" + row.id.instanceId + "\",\"" + row.indexType + "\")'>"+val+"</a>"
				}
			},{
				display : '指标标识',
				name : 'id.instanceId',
				...
				render : function(row,index,val){
					...
					if(row.sts == "03" || row.sts == "04" || row.sts == "07" || row.sts == "08"){
						return "<a style='color:blue' onclick='f_redo(\""+row.id.instanceId+"\",\""+row.id.dataDate+"\",\""+row.taskType+"\",\""+row.indexType+"\")'>重跑</a>";
					}
				}
			}],
			dataAction : 'server', 
			...
			alternatingRow : true,
			colDraggable : true,
			url : "${ctx}/report/frame/enginelog/idx/getEngineIdxStsList.json",
			sortName : 'startTime',
			sortOrder : 'desc', 
			pageParmName : 'page',
	...
	function queryEngineSts(){
		...
		$.ajax({
			cache : false,
			async : false,
			url : "${ctx}/report/frame/enginelog/idx/getEngineIdxPendingSts?d="+new Date(),
			dataType : 'json',
			type : "GET",
...
*/
	...
	function f_open_add() {
		dialog = BIONE.commonOpenDialog("新建指标引擎信息",
				"rptEngineBatchBox",800,500,
				"${ctx}/report/frame/enginelog/idx/newIdxEngineLog", null);
	}
	
	function  f_open_view(taskNo, dataDate, instanceId, indexType){
		dialog = BIONE.commonOpenDialog("指标任务状态",
				"rptEngineDetail",900,500,
				"${ctx}/report/frame/enginelog/idx/detailLog?taskNo=" 
						+ taskNo + "&dataDate=" + dataDate + "&instanceId=" + instanceId + "&indexType=" + indexType, null);
	}
	//新增影响指标跑数
	function addDependencyIdx() {
		dialog = BIONE.commonOpenDialog("新增影响指标跑数",
				"addDependencyIdxDialog",800,500,
				"${ctx}/report/frame/enginelog/idx/addDependencyIdx", null);
	}
	...
	function f_redo(instanceId,dataDate,taskType, indexType){
