...
<script type="text/javascript">
	...
	var selCell = currSheet ? currSheet.getCell(selRow , selCol) : null;
	var customgrid = null;
	var innergrid = null;
	var mainform=null;
	$(function() {
	...
	function f_save(){
		if(typeof window.parent.selCellSettingHandler == "function"){			
			window.parent.selCellSettingHandler($("#info").val() , cellType);
		}
		BIONE.closeDialog("moduleClkDialog");
	}
	function initCustom(){
	...
	function initForm() {
		...
		if(selCell){
			var currVal = selCell.value();
			if(currVal != null
					&& currVal != ""){
				$("#mainform textarea[name=info]").val(currVal);
			}
		}
	}
	function initTab() {
	        ...
	        g.gridbody.add(g.f.gridbody).ligerDrag({ revert: true, animate: false,
	            ...
	            onStopDrag: function (current, e)
	            {
	                ...
	                if (flag == 1)  
	                {
	               		...
	               		var curValue = $("#info").val();
	               		var appendValue = "";
	               		for(var j = 0 ; j < rows.length ; j++){
	               			appendValue = appendValue + "$"+rows[j].varNo+"$";
	               		}
	               		$("#info").val(curValue+appendValue);
	                }
	                if (flag == 2 ) 
	                {
	               		//考虑到浏览器兼容问题，目前暂只支持在末尾追加
