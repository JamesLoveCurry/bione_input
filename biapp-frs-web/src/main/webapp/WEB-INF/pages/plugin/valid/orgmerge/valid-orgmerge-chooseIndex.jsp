...
var type = '${type}';
$(function(){
	initDatas(templateId);
});
function initDatas(templateId){
	if(templateId != null && templateId != "" && typeof templateId != "undefined" && templateId){
		// 2.设计器信息
		$.ajax({
			cache : false,
			async : true,
			...
			success : function(result){
				if(result != null && typeof result.tmpInfo != "undefined"){
...
function spreadDbclkCell(sender , args , rptIdxTmp){
	if(rptIdxTmp.realIndexNo && rptIdxTmp.realIndexNo != ""){
		if(type == "sum"){
			window.parent.frames["orgMergeAdd"].liger.get("sumIndexNoCombo").setValue(rptIdxTmp.realIndexNo);
			window.parent.frames["orgMergeAdd"].liger.get("sumIndexNoCombo").setText(rptIdxTmp.realIndexNo);
		}else if(type == "branch"){
			window.parent.frames["orgMergeAdd"].liger.get("branchIndexNoCombo").setValue(rptIdxTmp.realIndexNo);
			window.parent.frames["orgMergeAdd"].liger.get("branchIndexNoCombo").setText(rptIdxTmp.realIndexNo);
		}
		BIONE.closeDialog("chooseIndex");
	}else{
		BIONE.tip("请选择指标单元格");
	}
}
