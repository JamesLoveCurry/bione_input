	...
	tabObj=$("#tab").ligerTab({
		...
		onAfterSelectTabItem : function(tabID){
			if(flag&&tabID=='impupload'&&$("#impupload").attr('src')==''){
				$("#impupload").attr({src:"${ctx}/report/frame/detailtmp/impUpload"});
				tabChangeFlag=false;
			}
			if(flag&&tabID=='impgrid'&&$("#impgrid").attr('src')==''){
				$("#impgrid").attr({src:"${ctx}/report/frame/detailtmp/impGrid"});
				tabChangeFlag=false;
			}
		}
...
function addTab(){
	...
	$("#dataset").attr({src:"${ctx}/report/frame/detailtmp/dataSet"});
