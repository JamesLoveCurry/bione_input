...
<script type="text/javascript">
	...
	function initData(){
		$.ajax({
			cache : false,
			async : true,
			url : "${ctx}/report/frame/taskrundata/getTaskRunDataChild",
			dataType : 'json',
			data:{
				taskNo : "${taskNo}"
			},
	...
	function initGrid() {
		grid = $("#maingrid").ligerGrid({
			...
			alternatingRow : true,
			colDraggable : true,
			url : "${ctx}/report/frame/taskrundata/getTaskRptIdxChild.json?taskNo=${taskNo}",
			sortName : 'startTime',
			sortOrder : 'desc', 
			pageParmName : 'page',
	...
	function  f_open_view(taskNo, taskType){
		var dialogNm='';
		if(taskType=="RptIdGroup") {dialogNm="报表子任务状态";}
		if(taskType=="IndexNoGroup") {dialogNm="指标子任务状态";}
		dialog = BIONE.commonOpenDialog(dialogNm,
				"taskRunDataDetailChildLog",600,500,
				"${ctx}/report/frame/taskrundata/detailChildLog?taskNo="
						+ taskNo , null);
	}
</script>
</head>
