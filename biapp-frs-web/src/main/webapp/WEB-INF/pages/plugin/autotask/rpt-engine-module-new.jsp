...
<script type="text/javascript">
	...
	$(function() {
		...
		var btns = [{
			text : "取消",
			onclick : function() {
				setTimeout('BIONE.closeDialog("newModuleTaskBatchBox");',0);
			}
		}, {
			text : "运行",
			onclick : function() {
		...
		$($(".l-btn-hasicon")[0]).click(function(){
			  var   setNm =  $("#search").find("input[name=setNm]").val();
			  data.searchNm = setNm;
			  BIONE.loadTree("${ctx}/rpt/frame/rptenginetsk/getRowModuleTree?d=" + new Date().getTime(),leftTreeObj,data);
	...
	function initMainForm(){
	 ...
	 function  f_save(){
		 ...
		 $(nodes).each(function (index, domEle) { 
			  if(domEle.params.type=="02"){//叶子节点
				  checkedSetIds += domEle.params.id+",";
			  }
		  });
		...
		// parent.changeRefreshSts();//当点击运行，立即该表刷新状态，默认10秒
		 $.ajax({
				...
				dataType : 'json',
				data : {
					checkedTaskIds:   checkedSetIds    ,
					dataDate:          dataDate   ,
					taskType:          taskType
				},
				success : function(result) {
					window.parent.grid.loadData();
					BIONE.closeDialog("newModuleTaskBatchBox");
				},
				beforeSend : function() {
 					BIONE.loading = true;
