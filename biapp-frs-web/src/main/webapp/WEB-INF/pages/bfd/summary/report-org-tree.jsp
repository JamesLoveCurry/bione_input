...
<script type="text/javascript">
	...
	function initTree() {
		settingAsync = {
			async : {
				type : 'get',
				enable : true,
				url : "${ctx}/bfd/datasummary/getTree?orgType=${orgType}&t="
						+ new Date().getTime(),
	...
	function setTree(searchFlag){
		if(searchFlag || isOrgReport){
			...
			loadTree("${ctx}/bfd/datasummary/searchOrgTree?orgType=${orgType}&t=" + new Date().getTime(), leftTreeObj, {orgNm : $("#treeSearchInput").val(), isOrgReport : isOrgReport});
		}else{
	...
	function initBtn() {
		btns.push({
			...
			onclick : function() {
				...
				var c = window.parent.jQuery.ligerui.get("rptOrgNo");
	...
	function checkOrg(){
		...
		settingAsync.async.url = "${ctx}/bfd/datasummary/getTree?orgType=${orgType}&t=" + new Date().getTime() + "&isOrgReport=" + isOrgReport;
		setTree($("#treeSearchInput").val() != "");
		check();
	}
</script>
