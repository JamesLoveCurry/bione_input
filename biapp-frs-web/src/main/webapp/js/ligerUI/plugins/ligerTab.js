...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        moveToCurrentTabItem: function ()
        {
            /* 
             * @Revision 20130614135900-002 
             * liucheng2@yuchengtech.com
             * BUG描述：当tab过多时，会产生左右两个按钮；此时如果重新打开一个已存在、且被遮住的tab，
             * 	tab的内容已经正确显示，而tab的头仍旧是被遮挡状态。
             * BUG解决：新增了此方法（moveToCurrentTabItem），并在中进行调用！
             */
        	var g = this, p = this.options;
            var btnWitdth = $(".l-tab-links-left", g.tab.links).width();
            var leftList = new Array(); // 记录每个tab的left,由左到右
            ...
            $("li", g.tab.links).each(function (i, item)
	        {
	            ...
	            if (g.getSelectedTabItemID() == $(this).attr("tabid")) 
	            {
	            	myLeft = currentItemLeft;
            ...
            if (g.setTabButton()) 
            {
            	if (myLeft > currentLeft && myLeft < (currentLeft + mainwidth - myWidth)){
	            	// do-nothing
            	} else if (myLeft <= currentLeft) {
	            	g.tab.links.ul.animate({ left: -1 * parseInt(myLeft) });
	            } else if (myLeft >= (currentLeft + mainwidth - myWidth)) {
	            	g.tab.links.ul.animate({ left: -1 * parseInt(myLeft + myWidth - mainwidth + 40) });
	            }
            }
        },
        ...
        selectTabItem: function (tabid)
        {
            ...
            /*
             * @Revision 20130614135900-001 
             * liucheng2@yuchengtech.com
             * 参见：@Revision 20130614135900-002
             */
            if (g.isTabItemExist(tabid))
            {
            	g.moveToCurrentTabItem();
            }
            /* @Revision 20130614135900-001 END */
        },
        //移动到最后一个tab
