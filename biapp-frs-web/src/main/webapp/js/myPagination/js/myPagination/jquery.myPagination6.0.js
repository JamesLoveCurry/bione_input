	...
	function init(param, obj) {
		...
		function createPageBar() {
			...
			var b = opts.currPage,
				...
				e = opts.pageNumber,
				f = "";
			opts.panel.first_on && (f = "<a href='" + a + "' paged='1'>" + opts.panel.first + "</a>"), opts.panel.prev_on && (f += 1 == b ? '<span class="disabled" paged="' + opts.panel.prev + '">' + opts.panel.prev + " </span>" : "<a href='" + a + "' paged='" + (b - 1) + "'>" + opts.panel.prev + " </a>");
			var g = lastPage = 1;
			...
			opts.panel.next_on && (f += b == c ? '<span class="disabled" paged="' + opts.panel.next + '">' + opts.panel.next + " </span>" : "<a href='" + a + "' paged='" + (b + 1) + "'>" + opts.panel.next + " </a>"), opts.panel.last_on && (f += "<a href='" + a + "' paged='" + c + "'>" + opts.panel.last + "</a>"), f += getPanelTipInfo(), debug(opts.id), debug("最终生成菜单："), debug(f), obj.html(f), obj.children(":text").keyup(function() {
				var a = $(this),
					b = $.trim($(this).val());
				if (0 != b.length) {
					var c = /^\+?[0-9][0-9]*$/;
