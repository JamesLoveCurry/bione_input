 ...
 * 
 */
define(['jquery', 'JqueryUI', 'dimParam/layout'], function() {
...
//                clickAccordion("comp");
                ...
                if ($that) {
                    ...
                    //触发还未离开焦点的input属性框change事件//现在没有属性区，不需要渲染
//                    g.propertyView.$("input:focus").trigger('change');
                    //删除属性区
//                    g.propertyView.remove();
                }
    ...
    function _addDelBtn (template, $selectedLi) {
        ...
        img.click(function () {
            ...
            //wjx20150531修改  防止用户去除日期维度
            /*if(!(model.attributes.name == "DATE" || model.attributes.name == "ORG" || model.attributes.name == "CURRENCY")){
            	template.removeComponent(model);
            	leftTreeObj.checkNode(leftTreeObj.getNodeByParam("id", model.attributes.name, null), false, false, false);
            }*/
            template.removeComponent(model);
        	leftTreeObj.checkNode(leftTreeObj.getNodeByParam("id", model.attributes.name, null), false, false, false);
            //修改完毕
         ...
         //wjx20150531修改  防止用户去除日期维度
        var model = template.getComponent($selectedLi.find("[cid]").attr("cid"));
        if(!(model.attributes.name == "DATE" )){
        	div.css("left", ($selectedLi.position().left + $selectedLi.width() - 8)).css("top", $selectedLi.position().top - 1);
       	 	div.append(img);
