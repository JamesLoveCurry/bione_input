...
var pageTitleField = "data-pageTitle";
...
function findTabPanelByTitle(title) {
    var $ele = null;
    $("#tab-content").find("div.tab-pane").each(function () {
        var $div = $(this);
        if ($div.attr(pageTitleField) == title) {
            $ele = $div;
            return false;//退出循环
        }
    });
...
function findIframeById(pageId) {
	var $ele = findTabPanel(pageId);
	if($ele === null){
		return null;
	}
    return $ele.children("iframe");
}

function findIframeByTitle(title) {
	var $ele = findTabPanelByTitle(title);
	if($ele === null){
		return null;
	}
    return $ele.children("iframe");
...
var addTabs = function (options) {
        ...
        if(options.url){
            if((options.url.indexOf("/https://") < 0) && (options.url.indexOf("/http://") < 0)){
         	   options.url = basePath + options.url;
            	 ...
            	 options.url = options.url.replace("/https://", "https://");
            	 options.url = options.url.replace("/http://", "http://");
             }
        }else{
        	return;
        }
    ...
    if (findTabPanel(pageId) === null) {
        ...
        var $title = $('<a href="javascript:void(0);"></a>').attr(pageIdField, pageId).attr(pageTitleField, options.title).addClass("menu_tab");
        ...
        //是否允许关闭
        if (options.close) {
            var $i = $("<i class='fa fa-remove page_tab_close' style='cursor: pointer' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).attr(pageTitleField, options.title).appendTo($title);
        ...
        var $tabPanel = $('<div role="tabpanel" class="tab-pane"></div>').attr(pageIdField, pageId).attr(pageTitleField, options.title);
        ...
        if (options.content) {
            ...
            var iframeHeight = App.getViewPort().height - $footer.outerHeight() - $header.outerHeight();
            if ($tabs.is(":visible")) {
            	iframeHeight = iframeHeight - $tabs.outerHeight();
            }
            
            var $iframe = $("<iframe></iframe>").attr("src", options.url).css("width", "100%").css("height", iframeHeight).attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId).attr(pageTitleField, options.title);
    ...
    }else{//tab页已存在，替换src重新加载
    	var iframeId = "iframe_" + pageId;
    	var _iframe = window.document.getElementById(iframeId);
        ...
        $menu_ul.on("click", "li.treeview a", function () {
            ...
            if ($a.next().size() == 0) {//如果size>0,就认为它是可以展开的
            	...
            	if(menuId){
        			$.ajax({
        				type : "POST",
        				url : App.getbasePath() + "/bione/syslog/func/saveLog",
        				dataType : 'json',
        				data : {'menuId':menuId},
        				success : function() {}
        			});
