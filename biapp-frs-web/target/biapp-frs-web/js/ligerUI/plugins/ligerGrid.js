...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        addNullRow: function (rowdata) {
        	/* @Revision 2013-5-28 
        	 * liucheng2@yuchengtech.com */
        	var g = this;
        	var stdWidth = g.gridheader.width();
        	var nullRow = [];
        	var totalWidth = "";
        	if(g.columns) {
        	...
        	var iTotalWidth = stdWidth;
        	if (totalWidth) {
        		if (totalWidth.substring(totalWidth.length-1)==",") {
        			totalWidth = totalWidth.substring(0, totalWidth.length-1);
        		}
    			var widths = totalWidth.split(",");
    			for (var i=0; i<widths.length; i++) {
    				var cWidth = widths[i];
    				if (cWidth) {
    					var iWidth = 0;
    					if(cWidth.indexOf("%")>=0) {
    						iWidth = stdWidth * parseInt(cWidth.substring(0, cWidth.indexOf("%"))) / 100;
    					} else {
    						iWidth = cWidth;
    					}
    					iTotalWidth += parseInt(cWidth);
    					nullRow.push('<td style="width:' + iWidth + 'px; " class="l-grid-row-cell " >');
            			nullRow.push('  <div ></div>');
            			nullRow.push('</td>');
    				}
    			}
    			iTotalWidth = parseInt(iTotalWidth);
        	}
        	var innerBody = [];
        	innerBody.push('<DIV style="width: ' + iTotalWidth + 'px" class="l-grid-body-inner">');
        	innerBody.push('<TABLE class="l-grid-body-table" cellSpacing="0" cellPadding="0">');
        	innerBody.push('<TBODY>');
        	innerBody.push('<TR >');
        	innerBody.push(nullRow.join(''));
        	innerBody.push('</TBODY>');
        	innerBody.push('</TABLE>');
        	innerBody.push('</DIV>');
        	
        	g.gridbody.html(innerBody.join(''));
