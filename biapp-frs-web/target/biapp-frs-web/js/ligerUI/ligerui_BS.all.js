...
(function ($)
{
        ...
        return $.extend({
            create: function (container, editParm, controlOptions)
            {
                ...
                if (editParm.field)
                {
                    ...
                    if (!e.body)
                    {
                        //modify by tanxu at 2014-5-23 修改下拉框、日期框、弹出控件的comboboxName为空时候, name为undefined的问题，遗留代码需要保证id和name一样。
                        // var inputName = prefixID + txtInputName;
                        // var inputId = new Date().getTime();
                        var inputName = (!!txtInputName ? prefixID + txtInputName : new Date().getTime());
                        var inputId = inputName;
                        //modify end
                        //modify by tanxu at 2014-6-11 把attr树形传给具体的构造方法，方便combobox和popup等构造隐藏域的attr
    ...
    //jQuery version fix
    // modify by tanxu 2014-6-9 默认当前BIONE系统中的jquery 1.7.1的on和live函数不一样，on函数无法动态绑定事件
    // 高版本的jquery去掉live时再用on替代
    // $.fn.live = $.fn.on ? $.fn.on : $.fn.live;
    $.fn.live = $.fn.live ? $.fn.live : $.fn.on;
    //modify end
    if (!$.browser)
    {
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        valueFieldCssClass : null,
        //add by wjx at 2015-11-05
        dataFilter : false //是否启用联想过滤功能
        //
    };
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (this.element.tagName.toLowerCase() == "input")
            {
                ...
                //2015-11-05 wjx
                if(p.dataFilter){
                    //2015-11-05 wjx 添加下拉框可输入
                    this.element.readOnly = false;
                    $(this.element).live("focus",function(a,b,c){
                        if(g.data.length>0){
                            g.filter = $(this).val();
                            g.setData(g.data,$(this).val());
                        }
                    });
                    /* $(this.element).live("change",function(e){
                          if($(this).val()){
                              if(g.data.length>0){
                                  var val=g.findValueByText($(this).val());
                                  var text=g.findTextByValue("");
                                  if(g.selectedValue!=val||(val==""&&$(this).val()!=text)){
                                      //g.setValue(val);
                                  }
                              }
                          }
                    ...
                    //2015-11-06 wjx 添加下拉框联想过滤
                    $(this.element).bind("input propertychange",function(a,b,c){
                        if(!a.originalEvent.propertyName || a.originalEvent.propertyName == "value"){
                            if(g.curText != $(this).val()){
                                g.filter = $(this).val();
                                g.setData(g.data,$(this).val());
                                g.curText = $(this).val();
                                if(g.data && $(this).val()){
            ...
            if (p.valueFieldID)
            {
                ...
                //modify by tanxu at 2014-6-10 给下拉框隐藏域赋attr，避免检索没有attr设置的filed和op属性
                // if (g.valueField.length == 0) g.valueField = $('<input type="hidden"/>');
                if (g.valueField.length == 0) {
                    g.valueField = $('<input type="hidden"/>');
                    if(p.attr){
                        g.valueField.attr(p.attr);
                    }
                }
                //modify end
                g.valueField[0].id = g.valueField[0].name = p.valueFieldID;
            ...
            g.valueField.attr("data-ligerid", g.id);
            //开关
            g.link = $('<div class="l-trigger"><div class="l-trigger-icon icon-down" style="background: none;font-size: 12px;color: #3c8dbc;line-height: 1.8em;"></div></div>');
            ...
            {
                //modify by tanxu at 2014-5-28 修改点击空白处隐藏下拉框功能
                ...
                // if (tag == "html" || tag == "body")
                {
                    if($(_this.element).val()){
                        if(g.data.length>0){
                            var val=g.findValueByText($(_this.element).val());
                            if( g.selectedValue!=val){
                                g.setValue(val);
                            }
                        }
                    }
        ...
        _setSelectBoxHeight: function (height)
        {
            ...
            } else
            {
                ...
                {
                    //modify by weijiaxiang at 2014-6-10 修改当itemsleng==0时下拉框没有高度的问题
                    // if (itemsleng < 8)
                    if (itemsleng < 8 && itemsleng > 0)
                    //modify end
                    {
                ...
                if (value != null) {
                    ...
                    for (var i = 0; i < targetdata.length; i++) {
                        $("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(function () {
                            ...
                            $td.find("a").addClass("l-checkbox-checked");
                            $td.find("td").find(".l-hidden").attr("checked","checked");
                            this.checked = true;
                        });
                    }
                }
        ...
        clearContent: function ()
        {
            ...
            //modify by weijiaxiang at 2014-6-11 清除下拉框高度的同时重设高度
        ...
        setTree: function (tree)
        {
            ...
            //modify by tanxu at 2014-3-10 修改下拉框中无法获取树对象的问题
        ...
        //设置值到 文本框和隐藏域
        _changeValue: function (newValue, newText, isClose)
        {
            var g = this, p = this.options;
            ...
            try {
                // modify by xugy at 2015-08-20 使form 表单渲染后不获取焦点 begin
                // g.inputText.trigger("change").focus();
                g.inputText.trigger("change");
                // modify by xugy at 2015-08-20 使form 表单渲染后不获取焦点 end
            } catch(e) {
            }
        ...
        _checkboxUpdateValue: function ()
        {
            ...
            g._changeValue(valueStr, textStr,false);
        },
        _addClickEven: function ()
        {
            var g = this, p = this.options;
        ...
        _toggleSelectBox: function (isHide)
        {
            ...
            //modify by songxf at 2013-7-4 避免同一界面弹出过个菜单的问题
...
(function ($)
{
    ...
    $.ligerDefaults.DateEditor = {
        ...
        absolute: true,  //选择框是否在附加到body,并绝对定位
        cancelable: true      //可取消选择
        //, readonly: false              //是否只读  去掉和disabled冲突
    };
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (!g.inputText.hasClass("l-text-field"))
                g.inputText.addClass("l-text-field");
            g.link = $('<div class="l-trigger"><div class="l-trigger-icon icon-calendar1" style="background: none;font-size: 18px;color: #3c8dbc;margin-left: -2px;"></div></div>');
            ...
            }).click(function ()
            {
                ...
                g.bulidContent();
                if(p.showType == "month"){
                    $(".l-box-dateeditor-toolbar").find(".l-button-today").text("本月");
                }else if(p.showType == "year"){
                    $(".l-box-dateeditor-header-month").hide();
                    $(".l-box-dateeditor-header-btn.l-box-dateeditor-header-prevmonth").hide();
                    $(".l-box-dateeditor-header-btn.l-box-dateeditor-header-nextmonth").hide();
                    $(".l-box-dateeditor-toolbar").find(".l-button-today").text("本年");
                    $(".l-box-dateeditor-header-btn.l-box-dateeditor-header-prevyear").click(function(){
                        $("li", g.body.yearselector).each(function (i, item)
                        {
                            var currentYear = g.currentDate.year + (i - 4);
                            if (currentYear == g.currentDate.year)
                    ...
                    $(".l-box-dateeditor-header-btn.l-box-dateeditor-header-nextyear").click(function(){
                        $("li", g.body.yearselector).each(function (i, item)
                        {
                            var currentYear = g.currentDate.year + (i - 4);
                            if (currentYear == g.currentDate.year)
            ...
            //modify by tanxu at 2014-5-28 增加鼠标在日期控件外点击隐藏日期选择框功能
        ...
        showDate: function ()
        {
            ...
            var dateStr;
            if(p.showType == "date"){
                dateStr = this.currentDate.year + '/' + this.currentDate.month + '/' + this.currentDate.date + ' ' + this.currentDate.hour + ':' + this.currentDate.minute;
            }else if(p.showType == "month"){
                var day = new Date(this.currentDate.year,this.currentDate.month,0);
                dateStr = this.currentDate.year + '/' + this.currentDate.month + '/' + day.getDate() + ' ' + this.currentDate.hour + ':' + this.currentDate.minute;
            }else if(p.showType == "year"){
                var day = new Date(this.currentDate.year,this.currentDate.month,0);
                dateStr = this.currentDate.year + '/' + '1' + '/' + '1' + ' ' + this.currentDate.hour + ':' + this.currentDate.minute;
            };
...
(function ($)
{
    ...
    $(".l-dialog-tc .icon-close").live('mouseover', function ()
    {
        $(this).addClass("icon-close-over");
    }).live('mouseout', function ()
    {
        $(this).removeClass("icon-close-over");
    });
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            var tmpId = "";
            g.set(p, true);
            var dialog = $('<div class="l-dialog"><table class="l-dialog-table" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td class="l-dialog-tl"></td><td class="l-dialog-tc"><div class="l-dialog-tc-inner"><div class="l-dialog-icon"></div><div class="l-dialog-title"></div><div class="l-dialog-winbtns"><div class="l-dialog-winbtn icon-close"></div></div></div></td><td class="l-dialog-tr"></td></tr><tr><td class="l-dialog-cl"></td><td class="l-dialog-cc"><div class="l-dialog-body"><div class="l-dialog-image"></div> <div class="l-dialog-content"></div><div class="l-dialog-buttons"><div class="l-dialog-buttons-inner"></div></td><td class="l-dialog-cr"></td></tr><tr><td class="l-dialog-bl"></td><td class="l-dialog-bc"></td><td class="l-dialog-br"></td></tr></tbody></table></div>');
            $('body').append(dialog);
            g.dialog = dialog;
            g.element = dialog[0];
            ...
            if (p.allowClose == false) $(".icon-close", g.dialog).remove();
            ...
            //设置事件
            $(".l-dialog-tc .icon-close", g.dialog).click(function ()
            {
                if (p.isHidden)
                    g.hide();
                else
        ...
        recover: function ()
        {
            ...
            if (g.winmax)
            {
                ...
                if (p.slide)
                {
                    ...
                    //modify by songxf at 2013-6-25 因为dialog自带的按钮不太好用，没有在项目中使用，因此将按钮高度去掉了
        ...
        _setImage: function ()
        {
            ...
            if (p.type)
            {
                $(".l-dialog-image", g.dialog).addClass("dialog-size").css("background","none");
                if (p.type == 'success' || p.type == 'donne' || p.type == 'ok')
                {
                    $(".l-dialog-image", g.dialog).addClass("icon-succed").show();
                    g.dialog.content.css({ paddingLeft: 64, paddingBottom: 30 });
                }
                else if (p.type == 'error')
                {
                    $(".l-dialog-image", g.dialog).addClass("icon-failed").show();
                    g.dialog.content.css({ paddingLeft: 64, paddingBottom: 30 });
                }
                else if (p.type == 'warn')
                {
                    $(".l-dialog-image", g.dialog).addClass("icon-reduce").show();
                    g.dialog.content.css({ paddingLeft: 64, paddingBottom: 30 });
                }
                else if (p.type == 'question')
                {
                    $(".l-dialog-image", g.dialog).addClass("icon-information_2").show();
    ...
    $.ligerDialog.alert = function (content, title, type, callback, config)
    {
        content = content || "";
    ...
    $.ligerDialog.success = function (content, title, onBtnClick, config)
    {
        return $.ligerDialog.alert(content, title, 'success', onBtnClick, config);
    };
    $.ligerDialog.error = function (content, title, onBtnClick, config)
    {
        return $.ligerDialog.alert(content, title, 'error', onBtnClick, config);
    };
    $.ligerDialog.warn = function (content, title, onBtnClick, config)
    {
        return $.ligerDialog.alert(content, title, 'warn', onBtnClick, config);
    };
    $.ligerDialog.question = function (content, title, config)
    {
        return $.ligerDialog.alert(content, title, 'question', config);
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Filter.editors['date'] =
        {
            create: function (container, field)
            {
                var input = $("<input type='text'/>");
                container.append(input);
                input.dateEditor = input.ligerDateEditor(field.editor.options || {});
                return input;
            },
            setValue: function (input, value)
            {
                input.dateEditor.setValue(value);
            },
            getValue: function (input, field)
            {
                return input.dateEditor.getValue();
            },
            destroy: function (input)
            {
                input.liger('destroy');
    ...
    $.ligerDefaults.Filter.editors['popup'] = {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            ...
            var $ele=input.ligerPopupEdit(options);
            for(var i in field.editor.attr){
                $ele.valueField.attr(i,field.editor.attr[i]);
            }
            return input;
        },
        setValue: function (input, value)
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.set(p);
            //事件：增加分组
            $(document).on('click',"#" + g.id + " .addgroup", function ()
            {
                var jtable = $(this).parent().parent().parent().parent();
                g.addGroup(jtable);
            });
            //事件：删除分组
            $(document).on('click',"#" + g.id + " .deletegroup",function ()
            {
                var jtable = $(this).parent().parent().parent().parent();
                g.deleteGroup(jtable);
            });
            //事件：增加条件
            $(document).on('click',"#" + g.id + " .addrule",function ()
            {
                var jtable = $(this).parent().parent().parent().parent();
                g.addRule(jtable);
            });
            //事件：删除条件
            $(document).on('click',"#" + g.id + " .deleterole",function ()
            {
        ...
        addRule: function (jgroup)
        {
            ...
            $("select.fieldsel", rulerow).bind('change', function ()
            {
                ...
                //字段类型处理
                var fieldType = field.editor.type || "string";
        ...
        _bulidRuleRowHtml: function (fields)
        {
            ...
            var rowHtmlArr = [];
            var fieldType = fields[0].editor.type || "string";
        ...
        _bulidOpSelectOptionsHtml: function (fieldType)
        {
            ...
            for (var i = 0, l = ops.length; i < l; i++)
            {
                var op = ops[i];
                if(typeof op == "object"){
                    opHtmlArr[opHtmlArr.length] = '<option value="' + op.id + '">';
                    opHtmlArr[opHtmlArr.length] = op.text;
    ...
    $.ligerDefaults.Form.editors.textarea =
        {
            create: function (container, editParm, p)
            {
                ...
                if (editParm.field.validate) editor.attr("validate",liger.toJSON(editParm.field.validate));
    ...
    $.ligerDefaults.Form.editors.hidden =
        {
            create: function (container, editParm, p)
            {
                ...
                //modify by tanxu at 2014-5-30 添加options.attr属性的设置
                // editor.attr({
                //     id: id,
                //     name: id
                // });
                ...
                //modify end
                //modify by tanxu at 2014-5-30 添加设置初始化值options.value的设置
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 30, 40, 50, 100, 200],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        ...
        //modify by tanxu at 2014/8/18 修正表格高度的问题
//        rowHeight: 26,                           //行默认的高度
//        headerRowHeight: 28,                    //表头行的高度
        ...
        //modify by tanxu at 2014/8/18 修正表格编辑控件的位置问题
//        editorTopDiff : 0,                      //编辑器top误差
    ...
    {
        //modify by  songxf at 2013-6-26 修改BIONE中timestamp日期格式化的问题
        ...
        //modify end
        {
            ...
            var date = dateobj.time ? new Date(dateobj.time) : new Date(dateobj);
            //modify end
        ...
        //modify by songxf at 2013-6-26 修改表格中timestamp字段格式化的问题
        // if (value instanceof Date)
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            // modified by kanglg
            gridhtmlarr.push("                      	<div class='l-grid-body-inner'>");
            gridhtmlarr.push("                      	</div>");
            // end
            gridhtmlarr.push("                      </div>");
            gridhtmlarr.push("                  </div>");
            ...
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnfirst icon-previous2' style='color:#3c8dbc;font-weight:700;font-size: 15px;' title='首页'></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprev icon-back' style='color:#3c8dbc;font-weight:700;' title='上一页'></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:30px;line-height:15px'  /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnext icon-enter' style='color:#3c8dbc;font-weight:700; ' title='下一页'></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlast icon-next2' style='color:#3c8dbc;font-weight:700;font-size: 15px;'title='尾页'></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnload icon-refresh' style='color:#3c8dbc;font-weight:700;' title='刷新'></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        ...
        _setData: function (value)
        {
            ...
            if(g.currentData && g.currentData.Rows && g.currentData.Rows.length > 0){
                g.gridbody.css("background","none");
            }
        },
        //刷新数据
        ...
        */
        ...
        setCellEditing: function (rowdata, column, editing)
        {
            ...
            if (rowdata['__id'] != 0)
            {
                ...
                //modify by caiqinyu at 2014-4-1  当使用可编辑的editgrid带分组时，第一行的prev对象是分组行，不具备id等getRow方法中需要的信息
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                $('.l-bar-selectpagesize', g.toolbar).append("<select name='rp' style='height: 20px'>" + optStr + "</select>");
...
//            if (!data || !data.length)
            if (!data || data.length == undefined)
            {
                ...
                $("<div></div>").addClass("l-grid-body-inner").appendTo(g.gridbody).css({
                    ...
                    height: g.gridbody.height()+p.heightDiff
                });
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            {
                content = p.formatters[column.type].call(g, value, column, rowdata);// modify by kanglg on 2015/7/1
            }
            else if (text != null)
            {
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if ($("> div[position=left]", g.layout).length > 0)
            {
                ...
                g.left.header = $('<div class="l-layout-header"><div class="l-layout-header-toggle icon-back"></div><div class="l-layout-header-inner"></div></div>');
            ...
            g.leftCollapse = $('<div class="l-layout-collapse-left" style="display: none; "><div class="l-layout-collapse-left-toggle icon-enter"></div></div>');
            g.rightCollapse = $('<div class="l-layout-collapse-right" style="display: none; "><div class="l-layout-collapse-right-toggle icon-back"></div></div>');
        ...
        _onResize: function ()
        {
            ...
            if(typeof p.onEndResize == "function"){
                p.onEndResize();
            }
...
(function ($)
{
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item, menu)
        {
            ...
            if(item.icon){
                var aitem = $("<i class='icon-manage l-grid-icon' style = 'color : #3c8dbc;'></i>");
                aitem.removeClass("icon-manage");
                aitem.addClass("icon-" + item.icon);
                if(item.color){
                    aitem.css("color",item.color);
                }
                ditem.prepend(aitem);
            }
...
(function ($)
{
    ...
    $.ligerDefaults.Panel = {
        ...
        //edit by yuxuan
        onBeforeCollapse:null     //收缩panel事件
        //edit end
    };
    ...
    $.ligerui.controls.Panel.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.panel.bind("click.panel", function (e)
            {
                ...
                //edit by yuxuan 修改后整个panel头部都可以进行点击 伸缩
                if (jobj.hasClass("l-panel-header-toggle")|| jobj.hasClass("l-panel-header"))
                //edit end
                {
                    g.toggle();
        ...
        toggle : function()
        {
            ...
            {//edit by yuxuan 增加事件onBeforeCollapse判断
                if (g.trigger('beforeCollapse', [p]) == false){
                    return false;
                }
                //edit end
    ...
    $.ligerui.controls.PopupEdit.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (p.valueFieldID)
            {
                ...
                if (g.valueField.length == 0)
                //modify by tanxu at 2014-6-10 给弹出框隐藏域赋attr，避免检索没有attr设置的filed和op属性
                // g.valueField = $('<input type="hidden"/>');
                {
                    g.valueField = $('<input type="hidden"/>');
                    g.valueField.attr(p.attr);
            ...
            //开关
            g.link = $('<div class="l-trigger"><div class="l-trigger-icon icon-screening" style="background: none;line-height: 1.6em;color: #4bbdfb;padding-top: 5px;"></div></div>');
            //外层
            g.wrapper = g.inputText.wrap('<div class="l-text l-text-popup"></div>').parent();
            g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
            g.wrapper.append(g.link);
            g.wrapper.append(g.valueField);
            //modify by tanxu at 2014-3-21 修复popup控件没有data-ligerid的问题
            g.valueField.attr("data-ligerid", g.id);
            //end modify
            g.inputText.addClass("l-text-field");
            //开关 事件
...
(function ($)
{
    ...
    $.ligerDefaults.Tab = {
        ...
        moveToNew:false,            // 是否自动将焦点移动至新添加tab上
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        toggleSwitch: function (btn)
        {
            ...
            // modify by xugy at 2014-08-08 10:21
            // 对 windowsswitch 对象在原基础上增加绑定了 mouseleave 事件, 当鼠标下拉框时收起下拉框.
        },
        ...
        _addTabItemEvent: function (tabitem)
        {
            ...
            }).click(function ()
            {
                ...
                if (g.trigger('close',[tabid]) == false) return;
                g.removeTabItem(tabid);
            });
        ...
        _menuItemClick: function (item)
        {
            ...
            {
                case "close":
                    if (g.trigger('close',[g.actionTabid]) == false) return;
                    g.removeTabItem(g.actionTabid);
                    g.actionTabid = null;
                    break;
                case "closeother":
                    if (g.trigger('closeother',[g.actionTabid]) == false) return;
...
(function ($)
{
    ...
    $.ligerDefaults.TextBox = {
        ...
        //, readonly: false              //是否只读 去掉，会跟disabled冲突
    };
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            //modify by tanxu at 2014/8/19 修正文本框无法添加扩展属性的问题
            if (p.attr) {
                g.inputText.attr(p.attr);
            }
            //modify end
            g.set(p);
            g.checkValue();
        ...
        _setWidth: function (value)
        {
            if (value > 20)
            {
                ...
                this.inputText.css({ width: value - 10 });
            }
        },
        _setHeight: function (value)
        {
...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem : function(item) {
            ...
            {
                g.toolBar.append('<div class="l-toolbar-item l-toolbar-text"><span>' + item.text || "" + '</span></div>');
                // g.toolBar.append('<button type="button" class="btn btn-info">'+item.text+"123"+'</button>');
                return;
            }
            if (item.menu) {
                var menucontainer=$('<div class="l-menubar-item"></div>');
                var ditem = $('<div class="l-toolbar-item l-panel-btn"><span class="l-grid-span"></span><div class="l-panel-btn-l"></div><div class="l-panel-btn-r"></div><div class="l-menubar-item-down"></div></div>');
                ...
                // var ditem = $('<div class="l-toolbar-item l-panel-btn"><span class="l-grid-span"></span><div class="l-panel-btn-l"></div><div class="l-panel-btn-r"></div></div>');
                var ditem = $('<div class="l-toolbar-item l-panel-btn"><span class="l-grid-span" style="padding-left: 3px"></span><div class="l-panel-btn-l"></div><div class="l-panel-btn-r"></div></div>');
                g.toolBar.append(ditem);
...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        */
        delay: null
        //modify by tanxu at 2013-10-14 重新计算树结点宽度
        , maxNodeWidth : 0            //节点的最大宽度
        //modify end
    };
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (node, url, param,e)
        {
            ...
            //modify by tanxu at 2013-10-17 树无法设置ajaxType和parms的问题
            // param = param || [];
        ...
        selectNode: function (selectNodeParm)
        {
            ...
            $("li", g.tree).each(function ()
            {
                ...
                {
                    //modify by tanxu at 2014-3-21 修复多选时会取消的问题
                    // g.cancelSelect(this);
        ...
        //modify by tanxu at 2013-10-14 重新计算树结点宽度
        //计算最大结点的宽度
        _calMaxNodeWidth: function () {
            var g = this, p = this.options;
            p.maxNodeWidth = 0;
            var strObj = g._initCurrentStrObj(g._getFont());
            g._calMaxNodeWidthByData(g.getData(), strObj, 1);
            strObj.remove();
        },
        //通过数据递归计算结点的宽度，设置maxNodeWidth为最大宽度
        _calMaxNodeWidthByData: function (data, strObj, level) {
            var g = this, p = this.options;
            $(data).each(function (i, item) {
                var width = g._calSpanWidth(strObj, level, item.text);
                if (p.maxNodeWidth < width) {
                    p.maxNodeWidth = width;
                }
                if (this.children) {
                    g._calMaxNodeWidthByData(this.children, strObj, level + 1);
                }
            });
        },
        //计算整个span的宽度
        _calSpanWidth: function (strObj, level, text) {
            var g = this, p = this.options;
            var width = g._getCurrentStrWidth(strObj, text);
            var treeWidth = level * 22;
            if (p.checkbox) treeWidth += 22;
            if (p.parentIcon || p.childIcon) treeWidth += 22;
            return width + treeWidth;
        },
        //获取span上的字体样式对象
        _getFont: function(){
            var g = this;
            var font = {}, fontCssAttr=["font-size", "font-family", "font-weight"];
            var firstSpan = $("span:first", g.tree);
            if (firstSpan) {
                for(var i = 0; i < fontCssAttr.length; i++) {
                    font[fontCssAttr[i]] = firstSpan.css(fontCssAttr[i]);
                }
            }
            return font;
        },
        //初始化一个临时存放数据的dom对象
        _initCurrentStrObj : function (font) {
            var currentObj = $('<span>').hide().appendTo(document.body);
            for(var fontAttr in font){
                currentObj.css(fontAttr, font[fontAttr]);
            }
            return currentObj;
        },
        //获取指定文本的宽度
        _getCurrentStrWidth: function (strObj, text) {
            strObj.html(text);
            return strObj.width();
        },
        //modify end
        _upadteTreeWidth: function ()
        {
            ...
            //modify by tanxu at 2013-10-14 重新计算树结点宽度
            if (p.maxNodeWidth) {
                g.tree.width(p.maxNodeWidth + 22);
            } else {
                //modify end
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box"))
                    treeitem = $(obj).parent().parent();
                //modify by tanxu at 2013-10-21 修复树设置checkbox为false时，单击选择树中自定义icon图标报错的问题
                ...
                //modify by tanxu at 2013-10-14 树行的单击选中
                if(($(obj).attr("tagName") === undefined ? obj.tagName : $(obj).attr("tagName"))== "SPAN" && $(obj).parent().hasClass("l-body")){
                    obj = $(obj).parent().find(".l-checkbox");
                }
                //modify end
                //chekcbox even
        ...
        .ligerExtend(
            $.ligerui.core.UIComponent,
            {
                ...
                addItem : function(item) {
                     ...
                     * 配置属性 : operNo
                     */
                    if (item.operNo && top.window["protectedResOperNo"]) {

                        if ($.inArray(item.operNo, top.window["protectedResOperNo"]) != -1) {
                            // 说明改按钮收权限保护，需要判断当前登录用户是否具有权限
                            if (!top.window["authorizedResOperNo"]
                                || $.inArray(item.operNo, top.window["authorizedResOperNo"]) == -1) {
                                return;
                            }
                        }
                    }
                    ...
                    if (item.menu) {
                        ...
                        if (item.img) {
                            ...
                            // aitem.addClass("icon-" + item.icon);
                            aitem.addClass("fa " + item.icon);
                            ditem.addClass("l-treetoolbar-item-hasicon");///////////////////////////////////////////////////////////////////改这里修改图标目录
                        }
                        if(item.color){
                            aitem.css("color",item.color);
                        }
