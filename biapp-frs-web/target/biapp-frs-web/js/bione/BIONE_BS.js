...
(function($) {
    ...
    BIONE.createButton = function(options) {
        ...
        // <span></span></div>');
        var btnHtml = '<button type="button" class="btn btn-info" '
        	+'style="margin-left:10px;border-top-right-radius: 5px;'
        	+'border-bottom-right-radius: 5px;border-top-left-radius: 5px;'
        	+'border-bottom-left-radius: 5px;"></button>';
        // if (hasIcon) {
        //     btnHtml = btnHtml + ' btn-info';
        // }
        // btnHtml += '"></button>';
        var btn = $(btnHtml);
        var $i = null;
        if (p.icon) {
            $i = $('<i class="fa '+p.icon+'"></i>')
            btn.prepend($i);
        }
