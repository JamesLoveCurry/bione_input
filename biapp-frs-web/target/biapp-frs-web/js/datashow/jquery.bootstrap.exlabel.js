...
(function($){
	...
	$.extend(LabelItem.prototype, {
		...
		_render: function() {
			var t = this;
			var $dom = $('<div class="label"/>').attr({
				'lId': this.lId
			}).addClass(this.style || 'label-default');
...
//				$dom.addClass('close');
				var close = $('<i class="glyphicon glyphicon-remove"></i>').appendTo($dom);
				close.bind('click', this, t.close);
			}
			if (this.isCheck != null) {
				var icon = $('<i class="glyphicon glyphicon-remove"></i>').appendTo($dom);
				$dom.addClass('check');
	...
	$.extend(ExLabel.prototype, Base.prototype, {
		...
		addLabel: function(param, target, isBefore) {
			...
			var label = {
				content: $('<div class="label label-default"/>').attr('lId', data.lId),
