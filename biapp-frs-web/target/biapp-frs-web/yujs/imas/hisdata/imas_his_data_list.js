...
function AfterInit() {
	...
	str_className = "Class:com.yusys.imas.business.service.ImasModelTempletBuilder.getTemplet('" + tab_name + "','" + tab_name_en + "','" + str_LoginUserOrgNo + "','1')";

	JSPFree.createBillList("d1", str_className, null, {list_btns:"[icon-p81]查看/view1;",isSwitchQuery:"N",autoquery:"N",list_ischeckstyle: "Y",list_ismultisel: "Y", refWhereSQL: whereSql});
}
/**
 * 加载页面之后处理
 * @returns
...
function insert1() {
	...
	JSPFree.openDialog3("新增","/yujs/imas/busidata/imas_check_data_edit.js",1100,560,defaultVal,function(_rtdata){
		if (_rtdata == "CHECK_OK") {
...
function update1() {
	...
	//判断当前数据是否被锁定
	var lock_data = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasLockData", "getStatus", {tab_name:tab_name,data_dt:json_rowdata[0].data_dt,org_no:json_rowdata[0].rpt_org_no,type:'2'});
	...
	JSPFree.openDialog3("编辑","/yujs/imas/busidata/imas_check_data_edit.js",null,null,defaultVal,function(_rtdata) {
		if (_rtdata == "CHECK_OK") {
			JSPFree.alert("校验并保存成功!");
...
function view1() {
	...
	JSPFree.openDialog3("查看","/yujs/imas/busidata/imas_check_data_edit.js",null,null,defaultVal,function(_rtdata) {},true);
}
...
function exportCSV(){
	...
	var jso_rt = JSPFree.doClassMethodCall("com.yusys.bione.plugin.detail.download.service.DetailDownloadBS", "getMaxDataNumCsv",{type: "IMAS"});
...
function importExcel(){
	...
	JSPFree.openDialog("文件上传","/yujs/imas/busidata/imas_busi_data_import.js", 500, 360, {tab_name:tab_name, tab_name_en:tab_name_en},function(_rtdata){
...
function delete1(){
    ...
    for (var i=0; i<json_rowdata.length; i++) {
    	//判断当前数据是否被锁定
    	var lock_data = JSPFree.doClassMethodCall("com.yusys.imas.process.service.ImasLockData", "getStatus", {tab_name:tab_name,data_dt:json_rowdata[i].data_dt,org_no:json_rowdata[i].rpt_org_no,type:'2'});
    	if(lock_data.status == "锁定"){
    		$.messager.alert('提示', '当前日期的表数据已被锁定，无法操作！', 'warning');
    		return;
    	}
	...
	$.messager.confirm('提示', '你真的要删除选中的记录吗?', function(_isConfirm) {
		...
		try {
			...
			for(var i=0; i<json_rowdata.length; i++){
				...
				if (json_rowdata[i].data_ources!=ImasFreeUtil.getDataOurces().etl_process && json_rowdata[i].data_ources!=null && json_rowdata[i].data_ources!='undefined' && json_rowdata[i].data_ources!=''){
					var source_type;
					//删除新增
					if (dataOurces==ImasFreeUtil.getDataOurces().page_entry){
						source_type=ImasFreeUtil.getSourceType().page_entry;
					} else if (dataOurces==ImasFreeUtil.getDataOurces().batch_import) {
						source_type=ImasFreeUtil.getSourceType().batch_import;
					}
					...
					if (json_rowdata[i].data_modify!=""){
						var source_type;
						if (dataModify==ImasFreeUtil.getDataModefy().data_maintenance){
							source_type=ImasFreeUtil.getSourceType().data_maintenance;
						}else if (dataModify==ImasFreeUtil.getDataModefy().data_filling){
							source_type=ImasFreeUtil.getSourceType().data_filling;
						}
						var _par={tab_name_en:tab_name_en,data_dt:json_rowdata[i].data_dt,number:-1,source_type:source_type,rpt_org_no:json_rowdata[i].rpt_org_no};
						JSPFree.doClassMethodCall("com.yusys.imas.dataoperation.StatisticalOperation","statisticalOperationForForm",_par)
					}
					var _par={tab_name_en:tab_name_en,data_dt:json_rowdata[i].data_dt,number:-1,source_type:ImasFreeUtil.getSourceType().etl_process,rpt_org_no:json_rowdata[i].rpt_org_no};
					JSPFree.doClassMethodCall("com.yusys.imas.dataoperation.StatisticalOperation","statisticalOperationForForm",_par)
			...
			JSPFree.doClassMethodCall("com.yusys.imas.business.service.ImasBusinessBS", "del", {ds:str_ds, str_id:idStr, tab_name_en:tab_name_en,tabName: tab_name, jsonData: json_rowdata});
			// 从界面上删除行
