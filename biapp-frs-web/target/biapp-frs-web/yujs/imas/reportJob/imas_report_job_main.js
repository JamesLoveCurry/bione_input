...
function AfterInit(){
    ...
    JSPFree.createBillList("d1_A", "/biapp-imas/freexml/reportJob/imas_report_job_code.xml",null,{isSwitchQuery:"N"});
    JSPFree.createBillList("d1_B", "/biapp-imas/freexml/reportJob/imas_report_job_log_code.xml",null,{isSwitchQuery:"N"});
...
function startJob(){
    ...
    var jso_data = JSPFree.doClassMethodCall("com.yusys.imas.reportJob.service.ImasReportJobBS", "startJob", jso_par);
...
function endJob(){
    ...
    var jso_par = {jobName:selectDatas[0].name};
    var jso_data = JSPFree.doClassMethodCall("com.yusys.imas.reportJob.service.ImasReportJobBS", "stopJob", jso_par);
    if(jso_data.code == "success"){
...
function delete1(){
    ...
    JSPFree.confirm("提示", "你真的要删除选中的记录吗?", function(_isOK){
        if(_isOK){
            var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.reportJob.service.ImasReportJobBS", "deleteJob",{jobId:job.rid});
            if(jso_rt.code == 0){
 ...
 */
function onInsert() {
    JSPFree.openDialog("新增", "/yujs/imas/reportJob/imas_report_job_edit.js", 900, 560, {type:'ADD'}, function(_rtdata) {
        if (_rtdata == "保存成功") {
            JSPFree.alert("保存成功!");
        }
 ...
 */
function onUpdate() {
    var data = d1_A_BillList.datagrid('getSelected');
    if (data == null) {
        $.messager.alert('提示', '必须选择一条数据!', 'info');
        return;
    }

    var defaultVal = {rid: data.rid, type: 'EDIT'};
    JSPFree.openDialog("编辑", "/yujs/imas/reportJob/imas_report_job_edit.js", 900, 560, defaultVal, function(_rtdata) {
        if (_rtdata == "保存成功") {
            JSPFree.alert("编辑成功!");
        }
        JSPFree.queryDataByConditon(d1_A_BillList);
