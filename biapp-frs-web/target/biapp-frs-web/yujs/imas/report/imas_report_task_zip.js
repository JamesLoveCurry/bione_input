...
function doRefreshData(){
	var jso_statu = JSPFree.doClassMethodCall("com.yusys.imas.report.service.ImasCrReportBS", "checkZipAndDownloadStatu",{data_dt:str_date,org_no:str_org_no,report_type:str_report_type,loginOrg:str_LoginUserOrgNo});
 ...
 */
function onNext() {
	JSPFree.openDialogAndCloseMe("打包压缩下载一个机构某一日期下的所有报文","/yujs/imas/report/imas_report_task_choosedate.js", 400, 350,{data_dt:str_date,org_no:str_org_no,report_type:str_report_type,org_class:str_org_class,org_nm:org_nm});
}
 ...
 */
function onZip(){
	var jso_par ={org_no:str_org_no,data_dt:str_date,report_type:str_report_type,loginOrg:str_LoginUserOrgNo,rids:rids};
	JSPFree.confirm('提示', '你的要压缩报文吗,这是一个耗时操作,请谨慎操作!', function(_isOK){
		if (_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.imas.report.service.ImasCrReportBS", "zipReportFile",jso_par);
...
function onDownload(){
	JSPFree.confirm('提示', '你真的要下载压缩文件么?这是一个非常耗时的操作,请谨慎操作!<br>建议使用Ftp工具下载或直接在服务器端拷贝!', function(_isOK){
		if (_isOK){
			...
			var src = v_context + "/imas/report/download?org_no=" + str_org_no + "&loginOrg=" + str_LoginUserOrgNo + "&data_dt=" + str_date+"&report_type="+str_report_type ;
