...
function AfterInit(){
	...
	JSPFree.createBillList("d1","/biapp-cr/freexml/resultsummary/cr_result_tab_org_summary_ranking_v.xml");
	
	if (orgType == "Y") {
		if (orgClass == CrFreeUtil.getCrOrgClass().zh) {
			busiType = "1_" + orgNo;
		} else if (orgClass == CrFreeUtil.getCrOrgClass().fh) {
			busiType = "2_" + orgNo;
		}
	} else {
		busiType = "1_";
	...
	// 报送机构号转换成用户所属机构号
	var userNo = "";
	var jso_userorg = JSPFree.doClassMethodCall("com.yusys.cr.common.service.CrValidateQueryConditionBS", "getUserOrgNoCondition", {"orgNo" : orgNo});
	if (jso_userorg.msg == "ok") {
		userNo = jso_userorg.data;
	}
	
	var condition = "";
	var jso_org = JSPFree.doClassMethodCall("com.yusys.cr.common.service.CrValidateQueryConditionBS", "getQueryCondition", {"_loginUserOrgNo" : userNo});
	if (jso_org.msg == "ok") {
...
function historyFn(_btn) {
	...
	JSPFree.openDialog(row.tab_name,"/yujs/cr/resultsummary/cr_check_result_tab_ranking_org_history.js", 700, 450, jso_OpenPars, function(_rtdata){});
}