...
function AfterInit() {
	...
	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/person/crrs_person_personal_CODE1.xml",null,{list_btns:"[icon-add]新增/insert1;[icon-edit]编辑/update1;[icon-remove]删除/delete1;[icon-p67]下载模板/downModel;[icon-p68]导入/importPersonFun;[icon-p67]导出/downSingleFun;[icon-p31]查看/onView"
		,list_ischeckstyle:"N",list_ismultisel:"N",ishavebillquery:"Y",autoquery:"N",isSwitchQuery:"N"});

	d1_BillList.pagerType="2";
	var _sql = "select * from crrs_person_personal where "+ getQueryCondition(str_LoginUserOrgNo) +" order by data_dt desc";
...
function onQueryEvent(_condition) {	
	var _sql = "select * from crrs_person_personal ";
...
function importPersonFun(){
	JSPFree.openDialog("文件上传","/yujs/crrs/collection/importPerson.js", 500, 240, null,function(_rtdata){
		if (_rtdata == true) {
        	JSPFree.alert("上传成功");
...
function downModel(){
	...
	var src = v_context + "/crrs/common/downLoadModel?fileNm=25-PersionalInfo.xls";
 ...
 */
function insert1(){
	JSPFree.openDialog("新增","/yujs/crrs/collection/addPerson.js", 950,600, null,function(_rtdata){
...
function update1(){
	...
	JSPFree.openDialog2("修改","/yujs/crrs/collection/editPerson.js",950,600,{person:person},function(_rtdata){
...
function viewRule(){
	var jso_par1 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_PERSON_PERSONAL','crrs_person_personal_loan','crrs_person_joint_debtor','crrs_person_student_loan','crrs_person_ent_guaranteed') and ruletype='确定性校验'"};
	var jso_par2 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_PERSON_PERSONAL','crrs_person_personal_loan','crrs_person_joint_debtor','crrs_person_student_loan','crrs_person_ent_guaranteed') and ruletype='一致性校验'"};
	var jso_par3 = {list_btns:"",isSwitchQuery:"N",autoquery:"Y",ishavebillquery:"N",list_ispagebar:"Y",
			autocondition:"tablename_en in ('CRRS_PERSON_PERSONAL','crrs_person_personal_loan','crrs_person_joint_debtor','crrs_person_student_loan','crrs_person_ent_guaranteed') and ruletype='提示性校验'"};
...
function onView(){
	...
	JSPFree.openDialog2("基础数据","/yujs/crrs/dataquery/viewPerson.js",950,600,{person:person},true);
}
...
function delete1(){
	...
	JSPFree.confirm("提示", "你真的要删除选中的记录吗?", function(_isOK){
		if(_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.crrs.collection.service.CrrsCollectionBS", "deletePersonData",{rid:person.rid,customer_code:person.customer_code,data_dt:person.data_dt});
			if(jso_rt.msg == "OK"){
