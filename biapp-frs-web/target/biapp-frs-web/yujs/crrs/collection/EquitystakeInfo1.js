...
function AfterInit(){
    ...
    if(str_hasBaseBtn){
    	//创建列表
		JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/ent/crrs_ent_equitystake_CODE2.xml",null,{list_btns:"$VIEW;",autoquery:"N"});
    }
    else{
		if ("锁定" == str_status) {
			JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/ent/crrs_ent_equitystake_CODE2.xml",null,{list_btns:"$VIEW;",autoquery:"N"});
		} else {
			JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/ent/crrs_ent_equitystake_CODE2.xml",null,{autoquery:"N"});
		}
    }
