...
function AfterInit(){
	...
	JSPFree.createBillCard("d1_1","/biapp-crrs/freexml/crrs/ent/crrs_ent_trade_info_CODE1.xml",["保存/onSave/icon-ok"],null);
	document.getElementById("d1_2").innerHTML="<div style=\"width:100%;text-align:center;\">请先维护同业基本信息</div>";
	document.getElementById("d1_3").innerHTML="<div style=\"width:100%;text-align:center;\">请先维护同业基本信息</div>";

	JSPFree.setBillCardValues(d1_1_BillCard,{customer_type:'3'});
}
...
function onSave(){
	...
	if ("Y" == ischeck) {
		// 单条数据校验
		var backValue = JSPFree.editTableCheckData(d1_1_BillCard, "Add", "同业客户基础信息", "CRRS_ENT_TRADE_INFO","","2");
		if (backValue == "" || "undifind" == backValue) {
			...
			if(flag){
				...
				var tyData = JSPFree.getBillCardFormValue(d1_1_BillCard);
				JSPFree.openDialogAndCloseMe2("基础信息","/yujs/crrs/collection/editTy.js",900,560,{tyData:tyData},true);
			}
