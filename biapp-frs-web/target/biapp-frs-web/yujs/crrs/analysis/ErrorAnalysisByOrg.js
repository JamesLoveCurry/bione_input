...
function AfterInit(){
	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/rule/crrs_result_summary_CODE1.xml",null,{list_btns:"$VIEW2;[icon-p69]导出xls/onExport_1;",autoquery:"N"});
	JSPFree.billListBindCustQueryEvent(d1_BillList, onErrorSummary);
}

function onErrorSummary(_condition){
	var _sql ="select s.reportdate, s.inst_code,"+
	...
	_sql += " group by s.reportdate,s.inst_code";
	JSPFree.queryDataBySQL(d1_BillList, _sql);
}

function onExport_1(){
//	var str_sql = d1_BillList.CurrSQL;
//	JSPFree.downloadExcelBySQL("数据差错分析-机构.xls", str_sql, "数据差错分析-机构");
}