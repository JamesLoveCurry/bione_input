...
function AfterInit() {
	...
	JSPFree.createBillCard("d1","/biapp-crrs/freexml/crrs/engineTask/crrs_engine_task_CODE1.xml", ["保存/onSave/icon-ok"], null);
}
...
function AfterBodyLoad() {
	...
	JSPFree.setBillCardItemValue(d1_BillCard, "edition_type", "2");
	...
	} else if (org_class == CrrsFreeUtil.getCrrsOrgClass().fh) {
		JSPFree.setBillCardItemValue(d1_BillCard, "busi_org_no", org_no);
		JSPFree.setBillCardItemValue(d1_BillCard, "busi_type", "2");
		JSPFree.setBillCardItemEditable(d1_BillCard, "busi_org_no", false);
	} else if (org_class == CrrsFreeUtil.getCrrsOrgClass().zzh) {
		JSPFree.setBillCardItemValue(d1_BillCard, "busi_org_no", org_no);
		JSPFree.setBillCardItemValue(d1_BillCard, "busi_type", "3");
		JSPFree.setBillCardItemEditable(d1_BillCard, "busi_org_no", false);
	}
	
	var taskCmd = JSPFree.doClassMethodCall("com.yusys.crrs.engineTask.service.CrrsEngineTaskBS", "getTaskCmd", {taskType: '表任务'});
	...
	if ($("#busi_org_no") && $("#busi_org_no").length > 0) {
		$("#busi_org_no").textbox({
			onChange: function (newValue, oldValue) {
				var org_class = CrrsFreeUtil.getOrgClass(newValue)
				if (org_class == CrrsFreeUtil.getCrrsOrgClass().zh) {
					// 如果是总行，则进行赋值
					JSPFree.setBillCardItemValue(d1_BillCard, "busi_type", "1");
				} else if (org_class == CrrsFreeUtil.getCrrsOrgClass().fh) {
...
function onSave() {
	...
	if (org_class == CrrsFreeUtil.getCrrsOrgClass().zh) {
		// 校验报送机构类型和报送机构号是否匹配
		var jso_org = JSPFree.doClassMethodCall("com.yusys.crrs.engineTask.service.CrrsEngineTaskBS","checkOrgNoIsMatching", {busiOrgNo : str_busi_org_no});
		if (jso_org.msg == "OK") {
			if ((jso_org.orgClass==CrrsFreeUtil.getCrrsOrgClass().zh && str_busi_type=="1") 
					|| (jso_org.orgClass==CrrsFreeUtil.getCrrsOrgClass().fh && str_busi_type=="2")
					|| (jso_org.orgClass==CrrsFreeUtil.getCrrsOrgClass().zzh && str_busi_type=="3")) {
