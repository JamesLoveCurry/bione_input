...
function AfterInit(){
	...
	ary_title.push(getTitleText("集团成员","crrs_group_members",jso_ErrorCount));
	...
	JSPFree.createTabb("d1",ary_title,[100,100,100,120]);
	...
	JSPFree.createBillCard("d1_1","/biapp-crrs/freexml/crrs/group/crrs_group_group_client_CODE2.xml",ary_CardBtn,jso_CardConfig);
...
function onSelect(_index,_title){
	...
	if(newIndex==2){
		if(!isLoadTabb_2){
			JSPFree.createBillList("d1_2","/biapp-crrs/freexml/crrs/group/crrs_group_members_CODE1.xml",null,jso_ListConfig);  //高管及重要联系人
			$.parser.parse('#d1_2');
	...
	if(newIndex==3){
		if(!isLoadTabb_3){
			JSPFree.createBillList("d1_3","/biapp-crrs/freexml/crrs/ent/crrs_ent_credit_CODE2.xml",null,jso_ListConfig);  //重要股东及主要关联企业信息
			$.parser.parse('#d1_3');
	...
	if(newIndex==4){
		if(!isLoadTabb_4){
			JSPFree.createBillList("d1_4","/biapp-crrs/freexml/crrs/ent/crrs_ent_group_client_CODE2.xml",null,jso_ListConfig);  //授信信息
			$.parser.parse('#d1_4');
