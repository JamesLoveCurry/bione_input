...
function AfterInit(){
	...
	JSPFree.createBillList("d1_A", "/biapp-crrs/freexml/crrs/scheduled/crrs_pub_job_CODE.xml",null,{isSwitchQuery:"N"});
 	JSPFree.createBillList("d1_B", "/biapp-crrs/freexml/crrs/scheduled/crrs_pub_job_b_CODE.xml",null,{isSwitchQuery:"N"});
...
function startJob(){
	...
	var jso_data = JSPFree.doClassMethodCall("com.yusys.crrs.scheduled.CrrsJobBS","startJob",jso_par); 
	if(jso_data.code == 0){
...
function endJob(){
	...
	var jso_par = {jobName:selectDatas[0].name};
	var jso_data = JSPFree.doClassMethodCall("com.yusys.crrs.scheduled.CrrsJobBS","stopJob",jso_par); 
	if(jso_data.code == 0){
...
function delete1(){
	...
	var jso_data = JSPFree.getHashVOs("SELECT REPORT_NAME FROM crrs_report_xml_config where AUTOJOBTRIGGER='"+selectDatas.rid+"'");
	if(jso_data != null && jso_data.length > 0){
		var msg = "当前任务已被报文策略【";
		if(jso_data.length > 1){
			for(var i=0; i<jso_data.length; i++){
				str_report_name = jso_data[i].report_name;
				if(i==jso_data.length-1){
					msg = msg +str_report_name+"】关联，不能删除或取消关联后再删除！";
				}else{
					msg = msg +str_report_name+"，";
				}
			}
		}else{
			str_report_name = jso_data[0].report_name;
			msg = msg +str_report_name+"】关联，不能删除或取消关联后再删除！";
		}
	...
	JSPFree.confirm("提示", "你真的要删除选中的记录吗?", function(_isOK){
		if(_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.crrs.scheduled.CrrsScheduleBS", "deleteJobData",{rid:selectDatas.rid});
			if(jso_rt.msg == "OK"){
