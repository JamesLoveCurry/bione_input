...
function AfterInit(){
	...
	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/rule/crrs_rule_result_FeedBack.xml",null,{list_btns:"[icon-p47]导入/onImport;$VIEW",querycontion:str_sqlCons});  //确定性校验
...
function onImport(){
	...
	JSPFree.confirm("提示","你真的要导入么?",function(_isOK){
		if(_isOK){
		  doCommitUpdateDB("crrs_result_consistency","已导入",jsy_rids);
		}
	});
}
