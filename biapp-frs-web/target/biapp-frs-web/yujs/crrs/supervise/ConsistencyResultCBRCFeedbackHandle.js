//跨行一致性处理
function AfterInit(){
	var str_sqlCons = "result_status2 in ('已分发')";  //过滤条件

	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/rule/crrs_rule_result_FeedBack.xml",null,{list_btns:"[icon-p41]处理/onHandle;$VIEW",querycontion:str_sqlCons,list_ischeckstyle:'Y',list_ismultisel:'Y'});  //确定性校验
...
function onHandle(){
	...
	if(li_count>0){
		JSPFree.alert("选中的数据中有【"+li_count+"】条状态不是[已分发]！");
		return;
	}
	JSPFree.confirm("提示","你真的要处理么?",function(_isOK){
		if(_isOK){
		  doCommitUpdateDB("crrs_result_warn","已处理",jsy_rids);
		}
	});
}
