...
function AfterInit(){
	...
	JSPFree.createBillList("d1","/biapp-crrs/freexml/crrs/rule/crrs_result_warn_FeedBack.xml",null,{list_btns:"[icon-p51]分发/onDispatch;$VIEW",querycontion:str_sqlCons,list_ischeckstyle:'Y',list_ismultisel:'Y'});  //确定性校验
...
function onDispatch(){
	...
	JSPFree.confirm("提示","你真的要分发么?",function(_isOK){
		if(_isOK){
		  doCommitUpdateDB("crrs_result_warn","已分发",jsy_rids);
		}
	});
}
