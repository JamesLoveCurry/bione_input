...
function AfterInit(){
	...
	JSPFree.createTabb("d1_A", [ "新增&修改", "删除", "法人检核未处理", "网点检核未处理" ]);
...
function onSelect(_index,_title) {
	...
	if(newIndex==1){
		...
		if (iss == 'N') {
			str_classNameError = "Class:com.yusys.east.business.model.service.EastModelTempleteBuilderFull.getTemplet('"+tabName+"','"+tabNameEn+"','"+rpt_org_no+"','" + org_no+ "','" + distributeType  + "','"+d+"','2')";
			JSPFree.createBillList("d1_A_4",str_classNameError,null,{list_btns:"$VIEW;",isSwitchQuery:"N",autoquery:"N",list_ispagebar:"Y",ishavebillquery:"Y"});
			$.parser.parse('#d1_A_4');
			// 由于每次切换tab，都会重新查数据，导致很慢。所以先注释掉
			issSql = getSql(tabNameEnIssZ, '2');
			d1_A_4_BillList.pagerType = "1";
			JSPFree.queryDataBySQL(d1_A_4_BillList, issSql);
			JSPFree.billListBindCustQueryEvent(d1_A_4_BillList, onDetailInfoErrorSummaryIssued);
			FreeUtil.resetToFirstPage(d1_A_4_BillList); //手工跳转到第一页
 ...
 * @param _condition
 */
function onDetailInfoErrorSummaryIssued(_condition) {
	if (iss == 'Y') {
		$.messager.alert('提示', '当前日期下该机构的数据未检核,无错误数据！', 'warning');
		return;
	}
	if (_condition != "") {
		issSql = issSql + " and" + _condition;
	}
	JSPFree.queryDataBySQL(d1_A_4_BillList, issSql);
	FreeUtil.resetToFirstPage(d1_A_4_BillList); //手工跳转到第一页
}