...
function onSave(){
	...
	if ("Y" == ischeck) {
		if (type == "Add" && !saveFlag) {
			JSPFree.setBillCardValues(d1_BillCard, {rid:FreeUtil.getUUIDFromServer(), proc_sts: "2"});
			...
			if (backValue == "" || "undifind" == backValue) {
				...
				var par = {modifiedType:EastFreeUtil.getModifiedType().insert, newData: JSPFree.getBillCardFormValue(d1_BillCard), tabName: tabname}
