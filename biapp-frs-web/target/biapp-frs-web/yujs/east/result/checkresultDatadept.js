...
function AfterInit(){
	...
	JSPFree.createBillList("d1","/biapp-east/freexml/east/result/v_east_cr_summary7.xml");
...
function historyFn(_btn){
	...
	var jso_OpenPars = {};
	jso_OpenPars.line_nm = row.line_nm;
	jso_OpenPars.tab_name = tabName;
	JSPFree.openDialog(row.tab_name,"/yujs/east/checkresultInterDepartHistory.js", 700, 450, jso_OpenPars,function(_rtdata){});
}
/**
 * 导出
 * @returns
...
function exports(){
	...
	var src = v_context + "/east/interface/order/downloadDepart?tabName="
			+ tabName
			+ "&dataDt="
			+ dataDt;
