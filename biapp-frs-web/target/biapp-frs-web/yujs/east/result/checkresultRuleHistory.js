...
function query(jso_OpenPars, _par){
	...
	if(_par != null && _par != ""){
		jso_par = {"SQLWhere": "org_no = '"+orgNo+"' and rule_id='"+ruleId+"' and "+_par+" and "+condition};
	} else{
	    var date2 = getPre6Month(date1);
	   	var _par = "(data_dt >='"+date2+"' and data_dt<='"+date1+"') and "+condition;
		jso_par = {"SQLWhere": "org_no = '"+orgNo+"' and rule_id='"+ruleId+"' and "+_par};
	}
	
	var jso_data = JSPFree.doClassMethodCall("com.yusys.east.checkresult.summary.service.EastCheckResultAllHisBS","getRuleHistoryChart",jso_par); 
