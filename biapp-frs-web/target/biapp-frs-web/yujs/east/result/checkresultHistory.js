...
function query(jso_OpenPars, _par){
	...
	if(_par != null && _par != "") {
		jso_par = {"SQLWhere": "tab_name='"+tabName+"' and org_no = '"+orgNo+"' and "+_par+" and "+condition};
	} else {
		var date1 = dataDt;
	    var date2 = getPre6Month(date1);
	   	var _par = "(data_dt >='"+date2+"' and data_dt<='"+date1+"') and "+condition;
		jso_par = {"SQLWhere": "tab_name='"+tabName+"' and org_no = '"+orgNo+"' and "+_par};
	}
	
	var jso_data = JSPFree.doClassMethodCall("com.yusys.east.checkresult.summary.service.EastCheckResultBS","getHistoryChart",jso_par); 
