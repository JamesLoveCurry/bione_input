...
function AfterInit(){
	...
	JSPFree.createBillQuery("d1_A_B","east/result/east_cr_summary_q");
...
function query(jso_OpenPars, _par){
	...
	if(_par != null && _par != ""){
		jso_par = {"SQLWhere": "addr = '"+addr+"' and rule_id='"+ruleId+"'"+_par};
	} else{
	    var date2 = getPre6Month(date1);
	   	var _par = "(data_dt >='"+date2+"' and data_dt<='"+date1+"')";
		jso_par = {"SQLWhere": "addr = '"+addr+"' and rule_id='"+ruleId+"' and "+_par};
	}
	
	var jso_data = JSPFree.doClassMethodCall("com.yusys.east.checkresult.summary.service.EastCheckResultAllHisBS","getRuleHistoryChartAddr",jso_par);
