function AfterInit(){
	JSPFree.createTabb("d1",["按规则类型统计","类型重要程度交叉表","按重要程度统计"]);  //创建多页签
	//第1个页签
	JSPFree.createSplit("d1_1","上下",60);  
	JSPFree.createBillQuery("d1_1_A","/biapp-east/freexml/east/rule/east_cr_rule_ref.xml");
...
//			    },
	...
	JSPFree.createBillQuery("d1_2_A","/biapp-east/freexml/east/rule/east_cr_rule_weight.xml");
	...
	JSPFree.bindQueryEvent(d1_2_A_BillQuery,function(_par){
		  //console.log("查询框传入的查询条件[" + _par + "]");
		  var jso_par = {SQLWhere:_par, isChart:true};
		  var jso_data = JSPFree.doClassMethodCall("com.yusys.east.checkrule.rulesummary.service.RuleReportBS","getRuleWeightReport",jso_par); 
		  var str_html = jso_data.tablehtml;  //返回的html
		  var sb_html = "<div>" +
							"<div id='mainchart5'></div>" +
						"</div>" +
						"<div>" +
							"<div id='mainchart6'></div>" +
						"</div> " +
					    "<div>" +
...
//				    },
	...
	JSPFree.createBillQuery("d1_3_A","/biapp-east/freexml/east/rule/east_cr_rule_tabweight.xml");
	document.getElementById("d1_3_B").innerHTML="<div style=\"width:100%;text-align:center;\">请输入条件查询数据!</div>";

	JSPFree.bindQueryEvent(d1_3_A_BillQuery,function(_par){
		  //console.log("查询框传入的查询条件[" + _par + "]");
		  var jso_par = {SQLWhere:_par, isChart:true};
		  var jso_data = JSPFree.doClassMethodCall("com.yusys.east.checkrule.rulesummary.service.RuleReportBS","getTableWeightReport",jso_par); 
		  var str_html = jso_data.tablehtml;  //返回的html
		  var sb_html = "<div>" +
		  ...
		  var charTable = JSON.parse(str_html.substring(str_html.indexOf("{")));  
			...
			var myChart9 = echarts.init(document.getElementById('mainchart9'));
			// 折线、柱状图
			option9 = {
					title: {
				        text: '权重'
			...
			var myChart10 = echarts.init(document.getElementById('mainchart10'));
			option10 = {
					title: {
				        text: '表名'
				    },
			...
			var myChart11 = echarts.init(document.getElementById('mainchart11'));
			option11 = {
					title: {
				        text: '权重'
				    },
...
//				    },
			...
			var myChart12 = echarts.init(document.getElementById('mainchart12'));
			option12 = {
					title: {
				        text: '表名'
				    },
