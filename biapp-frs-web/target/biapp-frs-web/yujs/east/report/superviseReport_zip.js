...
function doRefreshData(){
	var jso_statu = JSPFree.doClassMethodCall("com.yusys.east.business.report.service.EastCrReportBSDMO", "checkZipAndDownloadStatu",{create_type:create_type,addr:addr,data_dt:str_date,org_no:str_org_no,create_type:create_type,addr:addr});
...
//压缩
function onZip(){
	var jso_par ={"org_no":str_org_no,"data_dt":str_date,"create_type":create_type,"addr":addr};
	JSPFree.confirm('提示', '你的要压缩报文吗,这是一个耗时操作,请谨慎操作!', function(_isOK){
		if (_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.east.business.report.service.EastCrReportBSDMO", "zipTarGzReportFile",jso_par);
...
function onDownload(){
	JSPFree.confirm('提示', '你真的要下载压缩文件么?这是一个非常耗时的操作,请谨慎操作!<br>建议使用Ftp工具下载或直接在服务器端拷贝!', function(_isOK){
		if (_isOK){
			...
			var src = v_context + "/east/report/download?org_no=" + str_org_no + "&data_dt=" + str_date+ "&create_type=" + create_type+ "&addr=" + addr;
