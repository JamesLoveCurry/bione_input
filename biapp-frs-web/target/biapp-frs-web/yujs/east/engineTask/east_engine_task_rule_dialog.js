//列表购物车参照,即可以实现翻页多选.
var ary_cartkeys = ['id', 'rule_name'];   //购物车的字段
...
function AfterInit(){
	...
	var whereSql = '';
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.east.common.service.EastGetReportBS", "getReportCNList", {});
	if (jsn_result.code == 'success') {
		whereSql = jsn_result.data;
	}
  //创建左边的表
  JSPFree.createBillList("d1_A","/biapp-east/freexml/east/engineTask/east_engine_rule_task_ref.xml",null,{isSwitchQuery:"N", refWhereSQL: whereSql});
  //计算出右边表格的列表与宽度
  ...
  for(var i=0;i<ary_cartkeys.length;i++){
	...
	//创建右边的购物车表
	JSPFree.createBillListByItems("d1_B","/biapp-east/freexml/east/engineTask/east_engine_rule_task_ref.xml",ary_cartkeys,ary_cartnames,ary_cartwidths);
  //双击左边表格数据,选择到右边!!
...
function onConfirm(){
	...
	JSPFree.closeDialog({"id": str_code, rule_name: str_value});
}
