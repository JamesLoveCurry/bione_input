...
function onSave(){
	...
	/*//校验是否重复入库
	var jso_data = JSPFree.getHashVOs("select count(*) c from east_cr_task where task_type='表任务' and data_dt='" + str_date + "' and busi_type='"+str_busi_type+"' and busi_org_no='"+str_busi_org_no+"'");

	if(jso_data != null && jso_data.length > 0 && jso_data[0].c > 0){
		JSPFree.alert("该机构，在日期[" + str_date + "]的数据已存在，请重新选择！");
		return;
	}*/
	JSPFree.setBillCardValues(d1_BillCard,{task_id:FreeUtil.getUUIDFromServer(),task_type:'表任务',status:'初始化',create_tm:getNowFormatDate,busi_type: str_busi_type});
...
function onSaveAddr(){
	...
	JSPFree.setBillCardValues(d1_BillCard,{task_id:FreeUtil.getUUIDFromServer(),task_type:'地区表任务',status:'初始化',create_tm:getNowFormatDate,busi_type: str_busi_type});
	var flag = JSPFree.doBillCardInsert(d1_BillCard);
	if(flag){
		JSPFree.closeDialog(flag);
	}
