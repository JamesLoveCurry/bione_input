...
var ds = "";
function AfterInit() {
	var str_className = jso_OpenPars2.templetcode;
	...
	// 赋值
	d1_BillCard.OldData = data;
	JSPFree.setBillCardValues(d1_BillCard, data);
...
function onSave() {
	...
	if (jso_rt != null && jso_rt.status == "锁定") {
		JSPFree.alert("日期[" + str_date
				+ "]的数据已经被锁定,不能保存!<br>请在【数据处理->数据锁定】中进行设置");
		return;
	}
	...
	/*var backValue = JSPFree.editTableCheckData(d1_BillCard, "Edit", tabname, tabnameen.toUpperCase(),ds,"5");
