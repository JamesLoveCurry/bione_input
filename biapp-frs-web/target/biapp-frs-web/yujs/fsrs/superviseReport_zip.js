...
function doRefreshData(){
	var jso_statu = JSPFree.doClassMethodCall("com.yusys.fsrs.report.service.FsrsCrReportBSDMO", "checkZipAndDownloadStatu",{data_dt:str_date,org_no:str_org_no});
	...
	if(jso_statu.code=="-999"){  //发生错误,则什么都不能干!
      ...
      $('#d1_onLookAllZipTaskAndFile').linkbutton('disable');
	} else{
...
function onZip(){
	...
	JSPFree.confirm('提示', '你的要压缩报文吗,这是一个耗时操作,请谨慎操作!', function(_isOK){
		if (_isOK){
			var jso_rt = JSPFree.doClassMethodCall("com.yusys.fsrs.report.service.FsrsCrReportBSDMO", "zipTarGzReportFile",jso_par);
...
function onDownload(){
	JSPFree.confirm('提示', '你真的要下载压缩文件么?这是一个非常耗时的操作,请谨慎操作!<br>建议使用Ftp工具下载或直接在服务器端拷贝!', function(_isOK){
		if (_isOK){
			...
			var src = v_context + "/fsrs/report/download?org_no=" + str_org_no + "&data_dt=" + str_date;
...
function onLookAllZipTaskAndFile(){
	JSPFree.openDialog("查看所有在线任务","/yujs/fsrs/superviseReport_lookZipFile.js",950,600);
}
