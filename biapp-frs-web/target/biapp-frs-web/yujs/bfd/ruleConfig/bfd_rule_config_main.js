...
function AfterInit() {
    var whereSql = '';
    var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.ruleconfig.service.BfdCrRuleBS", "getReportCNList", null);
    if (jsn_result.code == 'success') {
        whereSql = jsn_result.data;
    }
    JSPFree.createBillList("d1", "/biapp-bfd/freexml/ruleConfig/bfd_cr_rule_code.xml",null,{isSwitchQuery:"N",list_ischeckstyle:"Y",list_ismultisel:"Y", refWhereSQL: whereSql});
}
...
function importRule() {
    JSPFree.confirm("提示","规则导入会增量添加系统中的校验规则，请谨慎操作，请确认是否执行导入操作！",function(_isOK){
        if(_isOK){JSPFree.openDialog("文件上传", "/yujs/bfd/ruleConfig/bfd_input_rule.js", 500, 240, null,function(_rtdata) {
...
function updateRuleStsAndForceSubSts(column,sts) {
    ...
    var jso_data = JSPFree
        .doClassMethodCall(
            "com.yusys.bfd.ruleconfig.service.BfdCrRuleBS",
...
function onView(_this) {
    ...
    JSPFree.openDialog("规则查看", "/yujs/bfd/ruleConfig/bfd_rule_config_view.js", 750, 500,
...
function exportRule(_btn) {
    ...
    var src = v_context + "/bfd/rule/export?ruleId=" + ruleId
