...
function AfterInit() {
	...
	var templet = "/biapp-bfd/freexml/fillingProcess/bfd_lock_data_lock.xml";
	if(type=='2'){
		templet = "/biapp-bfd/freexml/fillingProcess/bfd_lock_data_lock1.xml";
	}
...
function onConfirm(){
    ...
    if(type=='1'){
    	jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdLockData", "startLockData", {rids:rid,status:jso_cardData.status});
    	jsoAlert(jso_rt);
    }else{
    	if(jso_cardData.status=='4' || jso_cardData.status=='5'){
    		var jso = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdLockData", "checkAuditData", {rids:rid});
        	if(jso.msg=='ok'){
        		jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdLockData", "unLockData", {rids:rid,status:jso_cardData.status});
        		jsoAlert(jso_rt);
        	}else{
        		// 需要弹窗确认
