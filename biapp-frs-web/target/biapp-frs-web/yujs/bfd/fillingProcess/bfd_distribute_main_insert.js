...
function AfterInit(){
	...
	JSPFree.createBillCard("d1_A","/biapp-bfd/freexml/fillingProcess/bfd_filling_process_CODE1_2.xml");  //卡片
	JSPFree.createBillList("d1_B","/biapp-bfd/freexml/common/bfd_tab_ref.xml",null,{isSwitchQuery:"N",ishavebillquery:"N", refWhereSQL: whereSql});
...
function AfterBodyLoad(){
	...
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "getDeptNo", null);
...
function onConfirm(){
	...
	maskUtil.mask();
	setTimeout(function () {
		var jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "batchInsertTask", jso_par);
		maskUtil.unmask();
...
function onConfirmAndDis(){
	...
	maskUtil.mask();
	setTimeout(function () {
		var jso_rt = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "onConfirmAndDis", jso_par);
		maskUtil.unmask();
		JSPFree.closeDialog(jso_rt);  //关闭窗口,并有返回值
	}, 100);
}
