...
function AfterInit(){
	...
	JSPFree.createBillList("d1", "/biapp-bfd/freexml/fillingProcess/bfd_filling_process_CODE1.xml", null, {
		querycontion: org_no,
 ...
 */
function insertTask(){
	JSPFree.openDialog("新增","/yujs/bfd/fillingProcess/bfd_distribute_main_insert.js",900,560,{org_no:org_no},function(_rtData){
		if (_rtData != null) {
...
function updateTask(){
	...
	JSPFree.openDialog("编辑","/yujs/bfd/fillingProcess/bfd_distribute_main_edit.js",700,560,jso_Pars,function(_rtData){
		if (_rtData == true) {
...
function deleteTask(){
	...
	JSPFree.confirm('提示', '你真的要删除选中的记录吗?', function(_isOK){
		if (_isOK){
			...
			var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "deleteByTaskRid", {allrids:jso_allrids,alltabnameens:jso_alltabnames});
...
function distributeTask(){
	...
	//判断当前数据是否被锁定
	for (var i = 0; i < json_data.length; i++) {
		var lock_data = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdLockData", "getStatus", {
	...
	setTimeout(function (){
		...
		var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "beforeChildTasks", str_json);
		maskUtil.unmask();
		if (jsn_result.msg == 'OK') {
...
function withdrawTask(){
	...
	JSPFree.confirm('提示', '撤回操作会清空子任务数据，请确认是否撤回？', function(_isOK){
		if (_isOK){
			maskUtil.mask();
			setTimeout(function () {
				var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "withdrawDataByTask", str_json);
				maskUtil.unmask();
				if (jsn_result.msg == 'OK') {
...
function childTask(){
	...
	if (json_data[0].status == '1' || json_data[0].status == '3') {
		JSPFree.openDialog2("子任务信息","/yujs/bfd/fillingProcess/bfd_distribute_main_child.js",900,600,jso_Pars,function(_rtData){
			if (_rtData != null) {
				if ("dirclose" == _rtData.type) {
					return;
				}
...
function unlock() {
	...
	var str_json = {jsonData:json_data}
	var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.process.service.BfdCrProcessBSDMO", "unlock", str_json);
	if (jsn_result.msg == 'OK') {
