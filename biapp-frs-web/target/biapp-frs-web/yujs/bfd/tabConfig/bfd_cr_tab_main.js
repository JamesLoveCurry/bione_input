...
function AfterInit(){
    ...
    JSPFree.createBillList("d1","/biapp-bfd/freexml/tabConfig/bfd_cr_tab_code.xml",null,{isSwitchQuery:"N", refWhereSQL: whereSql});
}
...
function deleteTab(){
    ...
    JSPFree.confirm('提示', '你真的要删除选中的记录吗?', function(_isOK){
        if (_isOK){
            ...
            var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.tabconfig.service.BfdCrTabBS", "deleteColsByTabName", {alltabs:jso_alltabs});
...
function checkCols() {
    ...
    var jso_Pars = {tabNameEn:json_data[0].tab_name_en,tabName:json_data[0].tab_name, dsName: json_data[0].dsName};

    var jsn_result = JSPFree.doClassMethodCall("com.yusys.bfd.tabconfig.service.BfdCrTabBS", "verificationData", jso_Pars);
...
function createTables() {
    ...
    $.messager.confirm('提示', '生成物理表如果已存在,则会删除原有数据,重新生成,是否继续?', function(_isConfirm) {
        ...
        var jso_data = JSPFree.doClassMethodCall("com.yusys.bfd.tabconfig.service.BfdCrTabBS", "createTables", {tableNames:tableNames});
...
function createAllTables() {
    $.messager.confirm('提示', '一键重新生成物理表,会删除原有数据,是否继续?', function(_isConfirm) {
        ...
        var jso_data = JSPFree.doClassMethodCall("com.yusys.bfd.tabconfig.service.BfdCrTabBS", "createAllTables");
        if (jso_data.code == "success") {
...
function maintainCols() {
    ...
    // 弹出窗口,传入参数,然后接收返回值!
    JSPFree.openDialog("表字段","/yujs/bfd/tabConfig/bfd_cr_col_main.js",1000,535,json_data[0],function(_rtdata){
...
function editSql() {
    ...
    // 弹出窗口,传入参数,然后接收返回值!
    JSPFree.openDialog("SQL维护","/yujs/bfd/tabConfig/bfd_cr_tab_sql.js",800,435,json_data[0],function(_rtdata){
        if (_rtdata == "保存成功") {
            JSPFree.alert("编辑成功!");
        }
        JSPFree.queryDataByConditon(d1_BillList);
...
function executeSql() {
    ...
    var jso_data = JSPFree.doClassMethodCall("com.yusys.bfd.tabconfig.service.BfdCrTabBS", "executeSql",{"sqlStr": strSql});
    if (jso_data.code == "success") {
