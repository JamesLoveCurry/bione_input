...
<script type="text/javascript">
	...
	function initTreeInfo(treeType, typ) {
		...
		if (!(canEdit && canEdit == "false")) { //可编辑状态下才进行查找
			$.ajax({
				cache : false,
				async : true,
				url : url,
		...
		if (taskId) {
			...
			if (exeobjs != null) {
				for ( var n in exeobjs) {
					...
					node.params.orgNo = exeobjs[n].id.exeObjId;
			...
			var tskobjs = parent.taskInfo.tskobjs;
			if (tskobjs != null) {
					...
					if (treeType == "org") {
						if (cascade) { //级联
							if (treeNode.getCheckStatus().checked) {
								...
								checkCascade(treeNode.id, checkOrgNodes,
										treeType); //当前节点所有下级节点            
							} else {
								removeCurrentNode(treeNode, checkOrgNodes);
								unCheckCascade(treeNode.id, checkOrgNodes,
										treeType);
							}
						} else { //非级联
							if (treeNode.getCheckStatus().checked) {
					...
					if (treeType == "org") {
						if (cascade) {
							if (treeNode.getCheckStatus().checked) {
								...
								for ( var i in tmpOrgNodes) {
									removeCurrentNode(tmpOrgNodes[i],
											checkOrgNodes); //移除当前节点
								}
							}
						...
						if (cascade) {
							if (treeNode.getCheckStatus().checked) {
								...
								removeSyncCascadeNodes(treeNode.id,
										checkRptNodes); //移除级联节点
							}
	...
	<div id="center" style="width: 278px; border: solid 1px #D0D0D0;">
		<div id="content" style="width: 278px;">
			...
			<div id="treeContainer"
				style="width: 278px; padding-top: 5px; overflow: auto; clear: both; background-color: #FFFFFF;">
				<ul id="tree"
