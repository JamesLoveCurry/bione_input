...
<script type="text/javascript">
	...
	//宁夏 START2020-4-27 18:33:33 进行校验判断 
	// 判断字符串是否为空
	...
	function initForm() {
		$("#mainform").ligerForm({
...

			{
				...
				options : {
					...
					isShowCheckBox : true,
					isMultiSelect : true,
					valueFieldID : 'disposeMode',
					data : $.extend({}, selection),
					onBeforeOpen : function() {
						liger.get('disposeMode').setData(selection);
					}
		...
		$("#statisticsDt").attr("readonly", "true");
		...
		buttons.push({
			text : '取消',
			onclick : function() {
				BIONE.closeDialog("perFlowEidtWin", null);
			}
		});
		if (id == "") {
			buttons.push({
	...
	//保存
	function f_save() {
		var bankAcctKindCd1 = $("#mainform [name='bankAcctKindCd']").val();
		if (orgNo != '9999') {
			if (bankAcctKindCd1 == '02' && orgNo != '3000') {
				BIONE.tip("您没有操作贷记结算账户的权限!");
				return;
			}
			BIONE.submitForm($("#mainform"), function() {
				...
				BIONE.closeDialog("perFlowEidtWin", "处理失败");
			});
		}
		BIONE.submitForm($("#mainform"), function() {
			BIONE.closeDialogAndReloadParent("perFlowEidtWin", "maingrid",
