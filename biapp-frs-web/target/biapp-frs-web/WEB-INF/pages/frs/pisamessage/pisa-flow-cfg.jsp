...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#maingrid").ligerGrid({
			...
			method : 'post',
			url : '${ctx}/frs/pisamessage/flowConfigList',
			sortName : 'outCode', //第一次默认排序的字段
			sortOrder : 'asc',
			toolbar : {}
		});
	}
	...
	function oper_add() {
		BIONE.commonOpenLargeDialog('新增流量流向信息', 'pisaFlowEidtWin','${ctx}/frs/pisamessage/pisaFlowEidt');
	}
	//修改属性
	function oper_modify() {
		...
		BIONE.commonOpenLargeDialog("修改流量流向信息", "pisaFlowEidtWin","${ctx}/frs/pisamessage/pisaFlowEidt?id="+rows[0].flowId);
	}
	//批量删除报文集
			...
			$.ligerDialog.confirm('您确定删除这' + ids.length + "条记录吗？", function(yes) {
				if (yes) {
					$.ajax({
						async : false,
						type : "post",
						url : '${ctx}/frs/pisamessage/deletePisaFlow?&ids=' + idsStr,
						success : function(res){
							BIONE.tip('删除成功');
							grid.loadData();
						},
	...
	function exportData(fileName,taskName,dataDate){
		var src = '';
		src = "${ctx}/frs/pbmessage/manager/exportDataInfo?&filepath="+fileName+"&taskNm="+taskNm+"&d="+ new Date();
		downdload.attr('src', src);
	};

	function exportXls(fileName){
		var src = '';
		src = "${ctx}/frs/pbmessage/exportXls?filepath="+fileName+"&d="+ new Date();
		downdload.attr('src', src);
	};
