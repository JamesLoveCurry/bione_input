...
<script type="text/javascript">
	...
	function initData(){
		var url="${ctx}/rpt/frame/idx/idxanalysis/rptList.json?id=${id}&type=${type}";
		$.ajax({
			cache : false,
			async : true,
			url : url,
					...
					sharpWidth : 150, //形状宽度
					 callback :{
				    	  ...
				    	  onDblClick : function(data){
				    		  if(data.data.rptId != "${id}"  && data.params.type == "rpt"){
				    			  window.top.analysisIndex.maintab.addTabItem({
										tabid: data.data.rptId,
										text: "报表【"+data.text+"】追溯分析",
										url: "${ctx}/rpt/frame/idx/idxanalysis/tab?id="+data.data.rptId+"&type=rpt&d="+new Date().getTime(),
										showClose: true
									});
								}
				    	  }
