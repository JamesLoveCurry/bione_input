...
<script type="text/javascript">
	...
	var exportTmp=function(fileName){
		var src = '';
		src = "${ctx}/report/frame/param/templates/exportTmpInfo?filepath="+fileName+"&d="+ new Date();
	...
	$(function() {
		...
		function initTree() {
			...
			var setting = {
				async : {
					enable : true,
					url : "${ctx}/report/frame/param/templates/getTree.json?t="
							+ new Date().getTime(),
					autoParam : [ "upId" ],
					dataType : "json",
					type : "post",
					dataFilter : function(treeId, parentNode, childNodes) {
						if (childNodes) {
							for ( var i = 0; i < childNodes.length; i++) {
								...
								childNodes[i].upId = childNodes[i].params.realId;
	...
	function exportTemplate(){
		BIONE.commonOpenDialog("参数模板导出", "exportWin", 600, 480,
		"${ctx}/report/frame/param/templates/exportParam");
	}
	
	function importTemplate(){
		BIONE.commonOpenDialog("参数模板导出", "importWin",600,380,
		"${ctx}/report/frame/param/templates/importParam");
	}
	...
	function zTreeOnClick(event, treeId, treeNode) {
		...
		if (currentNode != treeNode) {
			currentNode = treeNode;
			if (!tempGrid) {
				content = "<iframe frameborder='0' id='gridFrame' style='height:100%;width:100%;' src='${ctx}/report/frame/param/templates/grid?catalogId="
						+ treeNode.realId
						+ "&catalogName="
						+ treeNode.text
						+ "'></iframe>";
	...
	function addCatalog() {
		...
		if (currentNode != null)
			...
			url += "0";
		BIONE.commonOpenSmallDialog("新增参数模板目录", "editCatalogBox", url);
	}

	//修改分类
	function editCatalog() {
		if (!currentNode || currentNode.realId == "ROOT") {
			BIONE.tip("未选择任何目录。");
			return;
		}
		BIONE.commonOpenSmallDialog("修改参数模板目录", "editCatalogBox",
...
<body>
	...
	<div id="template.left.up">
		<span style="font-size: 12" id="left_up_span">参数模板目录</span>
	</div>
