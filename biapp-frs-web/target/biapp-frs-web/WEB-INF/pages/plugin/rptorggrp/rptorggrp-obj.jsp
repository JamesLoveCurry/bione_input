...
var orgNos;//修改时已经被勾选的机构列表
	$(function() {
		initTreeInfo();
		url='${ctx}/rpt/frame/rptorggrp/gettreelist?nodeType=${orgType}';
		var setting = {
				async : {
					enable : true,
					url :  url,
					autoParam : [ "orgNo","orgType" ],
					dataType : "json",
					enable : true,
					type : "post",
...
function f_save(){
	...
	$.ajax({
		cache : false,
		async : true,
		url : "${ctx}/rpt/frame/rptorggrp/saveorgobj",
		type : 'POST',
		data :{
			grpId : "${grpId}",
			ids : ids,
...
function getNodes(){
	$.ajax({
		...
		type:'POST',
		dataType:'json',
		url:"${ctx}/rpt/frame/rptorggrp/gettreeobj?grpId=${grpId}",
