...
<script type="text/javascript">
	var selectedIds = [];
	var iconsWidth = 31*26 + 3*3 + 33*4 + 72*3 + 32*1 + 14*4;
	var iconsHeight = 32*1;
	var iconsRows = 3*1;
	...
	function computeIconsRows() {
		if (CKEDITOR && CKEDITOR.config) {
			var ckWidth = CKEDITOR.config.width;
			if (!CKEDITOR.config.width) {
				ckWidth = $("#editor_div").width();
			}
			var mlc = Math.floor(iconsWidth / ckWidth);
			if (iconsWidth / ckWidth - mlc > 0) { mlc += 1; }
			iconsRows = mlc;
		}
	}
	function templateshow() {
    	...
    	$("#center").height($(document).height() - $("#bottom").height()-10);
		$("#editor_div").height(200);
		computeIconsRows();
		if (CKEDITOR && CKEDITOR.config) {
			CKEDITOR.config.height = $("#editor_div").height()-85;
		}
    }
    ...
    function loadAttachGrid(items, cols) {
    	...
    	// 
    	athGrid.push('<div id="attach_files" class="l-grid-body l-grid-body2 l-scroll" style="height: 80px;">');
    	...
    	for (var _idx in items) {
			...
			athGrid.push('              <img src="'+ getIcon(attach.attachType) +'" style="vertical-align:middle;" alt="" />&nbsp;'+ attach.attachName +'</div>');
			athGrid.push('          </td>');
    	}
...
<% 
	...
	settings.addConfigValue("width", "99%");
	settings.addConfigValue("resize_enabled", false);
	//settings.addConfigValue("align", "center");
...
<body>
	<div id="center">
		...
		<!-- baseinfo end -->
		<!-- "editor_div" begin -->
		<div id="editor_div" class="info_content" style="width: 98%; text-align:center; ">
			<ckeditor:editor textareaAttributes="<%=attr %>" 
				basePath="${ctx}/js/ckeditor/" config="<%=settings %>"
				editor="editor1" value="<%= value %>"/>
		</div>
		<!-- "editor_div" end -->
		<!-- attach_div begin -->
		<div id="attach_div" class="attach_div" style="width: 98%; text-align:left; ">
