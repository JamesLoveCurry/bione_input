...
define(['jquery', 'JSON2', '../template/property', '../template/util', 'Ligerui'], function ($, JSON2, Property, Util) {
	...
	function toLigerForm (componentes, viewType) {
		...
		for (i = 0 ; i < componentes.length ; i++) {
			...
			if(componentes[i]["type"] === "daterange" && "real" === viewType){
				fields.push(manager._buildHiddenField(componentes[i], "startDateId", viewType));
				fields.push(manager._buildHiddenField(componentes[i], "endDateId", viewType))
			}
		}
	...
	function resetDateValue() {
    	...
    	for (mid in managers) {
    		//edit by fangjuan 2016-05-19 下拉框的默认值和当前日期的同步
    		if ((managers[mid].type === "DateEditor" || managers[mid].type === "ComboBox") && /#\{([^}]+)/.test(managers[mid].options.value)) {
    			ids.push(mid);
    		}
    	}
    	if (ids.length > 0) {
    		require({
    			paths : {
    				...
    				for (var i = 0; i < ids.length ; i++ ) {
    					...
    					if (cal) {
    						if(widget.type === "ComboBox"){//下拉框 edit by fangjuan 2016-05-19
    							if(macro === 'now' || macro.indexOf('d')==0){//现在，和以d开都的，都按当"天"计算
    								widget.setValue(cal.dates());
    							}else if(macro.indexOf('M')==0 || macro.indexOf('month')==0){
    								widget.setValue(cal.months() + 1);
    							}else if(macro.indexOf('y')==0){
    								widget.setValue(cal.year());
    							}
    						}else{//日期框
	...
	$.extend(ViewValueManager.value, {
		...
		design : {
			...
			_buildHiddenField : function (componet, hiddenId, viewType){
				return null;
			},
			// 创建LigerForm的field对象
				...
				for (key in options) {
					if (options.hasOwnProperty(key)) {
						...
						if (key === 'type' && value === 'hidden') {
							field['display'] = '隐藏域';
							field[key] = 'text';
						}else if(key === 'type' && value === 'daterange'){
							field[key] = 'date';
						} else if(key === 'type' && value === 'dblin'){
							field[key] = 'select';
						}else if(key === 'type' && value === 'textrange'){
							field[key] = 'text';
						}else{
							field[key] = value;
						}
		...
		real : {
			...
			_buildField : function (component, viewType) {
				var componentType = Property.getComponentType(component['type']), field = null;
				field = ViewValueManager._createAttres(viewType, component, componentType, 'widget', {type : component["type"]});
				if ('select' === component['type'] || 'dblin' === component['type']) {
					...
					//edit by fangjuan 20151009
					if('dblin' === component['type'] && (!component['value'] || component['value'] == "")){
						field["options"]["onSuccess"] = function(data){
							if(data && data.length > 0){
								this._changeValue(data[0].id, data[0].text);
							}
						};
					}
					if('dblin' === component['type']){
						field.options["cancelable"] = false;
					}
				...
				if (requiredStr !== undefined && requiredStr !== null && requiredStr !== "" && (requiredStr === "true" || requiredStr === true)) {
...
//		                    			 ligerComp.inputText.val(item['text']);
			...
			setOptions : function (field, options) {
				...
				for (key in options) {
					if (options.hasOwnProperty(key) && options[key] !== undefined && options[key] !== null) {
						...
						//edit by fangjuan 20150909 如果是daterange，则手动改为text
						if (key === 'type' && options[key] === 'daterange') {
							field["type"] = 'date';
						}else if (key === 'type' && options[key] === 'dblin'){
							field["type"] = 'select';
						}else if (key === 'type' && options[key] === 'textrange'){
							field["type"] = 'text';
						}
					}
				}
	...
	$.extend(ViewValueManager.DataSeter, {
		...
		popup : {
				...
				if (options['url']) {
					$.ajax({
		                ...
		                success: function (data){
		                	...
		                	for(var i=0;i < vals.length;i++){
		                		...
		                		g._findTextInfo(data,vals[i],infos);
		                	}
		                	comp.setText(infos.join(";"));
		                }
		            });
				} else if (options['sql']) {
					data['sql'] = options['sql'];
					...
					$.ajax({
		                ...
		                success: function (data) {
		                	//edit by fangjuan 2016-04-13
		                	var infos = [];
		                	g._findTextInfo(data,value,infos);
		                	comp.setText(infos.join(";"));
		                }
		            });
				} else {
			...
			_findTextInfo : function(data, value,infos){
				var g = this;
				for(var i = 0; i < data.length; i++){
					if(value == data[i].id){
						infos.push(data[i].text);
					}
					if(data[i].children&&data[i].children.length>0){
						g._findTextInfo(data[i].children,value,infos);
					}
				}
			},
