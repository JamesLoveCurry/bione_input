 ...
 * </pre>
 */
public class Not implements IOper, Serializable {

	private static final long serialVersionUID = 5250762478816166780L;

	private static final String OPER_NAME = "!";

	private static final Integer ARG_SIZE = 1;
	...
	public Map<String, Object> parse(FelNode funcNode,
		...
		StringBuilder formulaContent = new StringBuilder(OPER_NAME);
		List<FelNode> childNodes = funcNode.getChildren();
		if (childNodes.size() != ARG_SIZE) {
			throw new FormulaException("[ " + OPER_NAME + " ]参数个数不合法");
		}
		FelNode fNode = childNodes.get(0);
		if (fNode instanceof ConstNode) {
			...
			if (!cellIdxsMap.containsKey(fText)) {
				throw new FormulaException("[ " + OPER_NAME + " ]运算中" + fText
						+ "单元格不是报表指标");
			}
			...
			if (!parseOver) {
				for (IOper operTmp : opers) {
					if (operTmp.nameEqual(fNode.getText())) {
						parseMapTmp = operTmp.parse(fNode, cellIdxsMap, funcs,
								opers);
						parseOver = true;
						break;
					}
			...
			List<String> idxsTmp = (List<String>) parseMapTmp.get("srcIndexNo");
			if(idxsTmp != null){				
	...
	public boolean nameEqual(String targetNm) {
		...
		if (!StringUtils.isEmpty(targetNm)
				&& targetNm.trim().toUpperCase().equals(OPER_NAME)) {
			flag = true;
		}
		return flag;
	}
