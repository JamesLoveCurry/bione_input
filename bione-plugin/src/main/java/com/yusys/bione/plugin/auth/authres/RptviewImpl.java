 ...
 */
@Component
public class RptviewImpl implements IResObject {

	@Autowired
	private RptMgrBS rptMgrBS;

	public static final String RES_OBJ_DEF_NO = GlobalConstants4plugin.RPTVIEW_RES_NO;
	...
	public List<CommonTreeNode> doGetResInfo() {
		...
		List<CommonTreeNode> bankNodes = this.rptMgrBS
				.getRptTreeAuth(
						com.yusys.bione.plugin.base.common.GlobalConstants4plugin.RPT_EXT_TYPE_BANK,
						null, "", true, true, null, null, false, false,
						GlobalConstants4plugin.RPTVIEW_RES_NO);
		allNodes.addAll(bankNodes);

		// 获取监管的报表树，展示invalid报表，不展示树根节点,不带权限过滤
		List<CommonTreeNode> monitorNodes = this.rptMgrBS
				.getRptTreeAuth(
						com.yusys.bione.plugin.base.common.GlobalConstants4plugin.RPT_EXT_TYPE_FRS,
						null, "", true, true, null, null, false, false,
						GlobalConstants4plugin.RPTVIEW_RES_NO);
