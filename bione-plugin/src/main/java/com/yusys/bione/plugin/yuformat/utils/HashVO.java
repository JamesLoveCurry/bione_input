...
/**
 * HashVO 是一个LinkedHashMap用来存储的数据结构
 * 可以根据列名取值，但可以自动转换想要的类型
 */
public class HashVO implements Serializable, Cloneable {

	private static final long serialVersionUID = -2432521772081284764L;

	private Map<String, Object> m_hData = new LinkedHashMap<String, Object>(); //LinkedHashMap按输入顺序排序,实际上就是在这个对象里存储了所有数据,但问题是每一行数据都会存储这么一个对象,数据量大时有一定瓶颈
	private String equalsFieldName = null; //
	private String toStringFieldName = null;
	private Map<Object, Object> ht_userobject = null; //
	...
	public String[] getKeys() {
		return m_hData.keySet().toArray(new String[0]);
	...
	public boolean containsKey(String _key) {
		return m_hData.containsKey(_key.toLowerCase());
	}
	...
	public String getStringValue(int _pos) {
		...
		if (str_keys != null && str_keys.length > _pos) {
			return getStringValue(str_keys[_pos]);
		} else {
			return null;
		}
	}
	...
	public Integer getIntegerValue(String attrname) {
		...
		if (o instanceof Integer) {
			...
			try {
				Integer li_return = new Integer(str_value.trim());
				return li_return; //
			} catch (Exception ex) {
				ex.printStackTrace(); //
				return 1;
	...
	public Boolean getBooleanValue(String attrname, boolean _nvl) {
		...
		if (o.toString().trim().equalsIgnoreCase("Y") || o.toString().trim().equalsIgnoreCase("yes") || o.toString().trim().equalsIgnoreCase("true") || o.toString().trim().equalsIgnoreCase("是") || o.toString().trim().equalsIgnoreCase("1")) {
			return Boolean.TRUE;
		} else if (o.toString().trim().equalsIgnoreCase("N") || o.toString().trim().equalsIgnoreCase("no") || o.toString().trim().equalsIgnoreCase("false") || o.toString().trim().equalsIgnoreCase("否") || o.toString().trim().equalsIgnoreCase("0")) {
			return Boolean.FALSE;
		}

		return Boolean.TRUE; // 先直接试下
	}
	...
	public Map<String, Object> getM_hData() {
		return m_hData;
	}

	public Map<Object, Object> getHt_userobject() {
		return ht_userobject;
	}

	public void setHt_userobject(Map<Object, Object> ht_userobject) {
		this.ht_userobject = ht_userobject;
	}

	public void setUserObject(Object _key, Object _value) {
		if (ht_userobject == null) {
			ht_userobject = new HashMap<Object, Object>();
		}
	...
	public String toString2() {
		...
		for (int i = 0; i < str_keys.length; i++) {
			sb_txt.append("【" + str_keys[i] + "】=【" + getStringValue(str_keys[i]) + "】\r\n");
		}
	 ...
	 */
	public Object convertToRealOBJ(Class<?> _class) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
