...
@ExcelSheet(index = "2", name = "格式校验", firstRow = 1)
public class RuleFormatImportVO implements Serializable, AnnotationValidable {
	...
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "J", name = "规则来源")
	private String ruleSource;
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "K", name = "约束类型")
	private String constraintType;
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "L", name = "展示分类")
	private String dispCd;
	@BioneFieldValid(nullable=true,comboVals = { "1", "2" }, length = 1)
	@ExcelColumn(index = "M", name = "规则业务类型", value = { "1", "2" }, text = { "全量SQL类型", "增量SQL类型" })
