...
@ExcelSheet(index = "0", name = "空值校验", firstRow = 1)
public class RuleEmptyImportVO implements Serializable, AnnotationValidable {
	...
	@ExcelColumn(index = "G", name = "前置条件")
	private String cond;
	@BioneFieldValid(comboVals = { "Y", "N" }, length = 1)
	@ExcelColumn(index = "H", name = "是否启用", value = { "Y", "N" }, text = { "启用", "停用" })
	private String ruleSts;
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "I", name = "规则来源")
	private String ruleSource;
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "J", name = "约束类型")
	private String constraintType;
	@BioneFieldValid(nullable=true, length = 20)
	@ExcelColumn(index = "K", name = "展示分类")
	private String dispCd;
	@BioneFieldValid(nullable=true,comboVals = { "1", "2" }, length = 1)
	@ExcelColumn(index = "L", name = "规则业务类型", value = { "1", "2" }, text = { "全量SQL类型", "增量SQL类型" })
