 ...
 */
public class UnitsUtil {// Each cell conatins a fixed number of co-ordinate points; this number
    ...
    */
    ...
    public static double widthUnits2Millimetres(short widthUnits) {
        return(UnitsUtil.widthUnits2Pixel(widthUnits) /
               UnitsUtil.PIXELS_PER_MILLIMETRES);
    }
    ...
    public static int millimetres2WidthUnits(double millimetres) {
        return(UnitsUtil.pixel2WidthUnits((int)(millimetres *
                UnitsUtil.PIXELS_PER_MILLIMETRES)));
    }
    /*
    ...
    public static int pixelsToPoints(double pixels) {
		return (int) Math.round(pixels *72D / PIXELS_PER_INCH);
	}
    */
     ...
     * pt与px转换方法保留小数位
     */
    public static float pointsToPixels(double points, boolean isRounding) {
    	if(isRounding) {
    		return (float) Math.round(points / 72D * PIXELS_PER_INCH);
    	}
		return (float) (points / 72D * PIXELS_PER_INCH);
	}
    
    public static float pixelsToPoints(double pixels, boolean isRounding) {
    	if(isRounding) {
    		return (float) Math.round(pixels *72D / PIXELS_PER_INCH);
    	}
		return (float) (pixels *72D / PIXELS_PER_INCH);
	}
