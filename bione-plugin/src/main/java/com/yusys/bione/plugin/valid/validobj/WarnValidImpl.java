 ...
 */
@Component
public class WarnValidImpl implements IValidTypeImpl {

	@Autowired
	private RptValidWarnBS rptValidWarnBS;
	...
	public List<CommonTreeNode> doGetValidIdxAsync(String groupId, String grpType, String validType, CommonTreeNode parentNode,
		...
		rstNodes.addAll(this.rptValidWarnBS.getValidIdxCtls(parentNode, basePath, validType));//目录节点
		rstNodes.addAll(this.rptValidWarnBS.getValidIdxs(parentNode, basePath, validType));//指标节点
		if("idxInfo".equals(parentNode.getParams().get("nodeType")) && 
				"05".equals(parentNode.getParams().get("idxType"))){//如果父节点是总账指标还要显示度量节点
			rstNodes.addAll(this.rptValidWarnBS.getIdxMeasure(parentNode, basePath, validType));//总账的度量节点
		}else{
			rstNodes.addAll(this.rptValidWarnBS.getIdxWarnValid(parentNode, basePath, validType));//警戒校验节点
		}
		return rstNodes;
	}
	...
	public Map<String, String> doGetAllValidTreeNode(String basePath, String groupId, String grpType,String validType) {
		...
		Map<String,Set<String>> warn = this.rptValidWarnBS.getWarnValidIdxAndGlMeasure();
		List<RptIdxInfo> warnIdxs = this.rptValidGroupBS.getValidIdxByNos(warn.get("allWarIdx"));
		if(null != warnIdxs && warnIdxs.size() > 0){
			List<String> oneLvlCtlNos = new ArrayList<String>();
			List<RptIdxCatalog> catalogs = new ArrayList<RptIdxCatalog>();
			for(RptIdxInfo idx : warnIdxs){//指标节点
	...
	public List<CommonTreeNode> doGetSearchSyncTreeNode(String basePath, String groupId,
		...
		List<CommonTreeNode> rstNodes = new ArrayList<CommonTreeNode>();
		List<CommonTreeNode> nodes = this.rptValidWarnBS.findSyncTreeByKeyWord(basePath, searchNm, validType);
